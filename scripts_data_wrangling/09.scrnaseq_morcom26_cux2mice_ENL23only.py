...
OUT_PCA_FILE  = os.path.join(OUT_RES_DIR, '09.pca_coords.tsv')
OUT_UMAP_FILE = os.path.join(OUT_RES_DIR, '09.umap_coords.tsv')
OUT_PCA_HTML  = os.path.join(OUT_FIG_DIR, '09.pca_celltype.html')
OUT_UMAP_HTML = os.path.join(OUT_FIG_DIR, '09.umap_celltype.html')
...
# --- filter to EN-L2-3-CTX and EN-L2-mix ---
mask = adata.obs[CELLTYPE_COL].astype(str).isin(KEEP_CELLTYPES)
adata = adata[mask].copy()
print(f'  {adata.shape[0]} cells after filtering to {KEEP_CELLTYPES}')
...
make_scatter_html(
    ...
    xlab='PC1', ylab='PC2',
    title='morcom26 CUX2 mice EN-L2-3-CTX & EN-L2-mix — PCA (colored by cell type)',
...
make_scatter_html(
    ...
    title='morcom26 CUX2 mice EN-L2-3-CTX & EN-L2-mix — UMAP (colored by cell type)',
