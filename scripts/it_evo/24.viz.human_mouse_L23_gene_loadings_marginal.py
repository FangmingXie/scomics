...
Outputs:
  local_data/fig/it_evo/24.human_mouse_L23_gene_loadings_CCA1.pdf
  local_data/fig/it_evo/24.human_mouse_L23_gene_loadings_CCA2.pdf
"""
...
OUT_PDF_CCA1  = os.path.join(FIG_DIR, '24.human_mouse_L23_gene_loadings_CCA1.pdf')
OUT_PDF_CCA2  = os.path.join(FIG_DIR, '24.human_mouse_L23_gene_loadings_CCA2.pdf')
...
def make_joint_figure(ms, hs, axis_label, out_pdf, sig):
    ...
    for L, color in ARCH_COLORS.items():
        m = letters == L
        ax_main.scatter(ms[m], hs[m], s=POINT_SIZE + 12, c=color, linewidths=0, alpha=0.9,
    ...
    ax_main.scatter(ms[top_idx], hs[top_idx], s=POINT_SIZE + 18, facecolors='none',
    ...
    ax_main.set_title(f'L2/3 {axis_label}: mouse vs human\n'
                      f'r = {r:.3f},  {p_str}  (gene-label permutation, n={N_PERM})',
                      fontsize=11)
...
print(f'--- L2/3 CCA gene-loading joint plots: {len(shared)} shared orthologs ---')
