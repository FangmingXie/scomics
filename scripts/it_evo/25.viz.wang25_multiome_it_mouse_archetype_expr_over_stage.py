...
Per-cell score (script-50 method): each ortholog-mapped marker gene's log2(CP10k+1) is
max-scaled to [0, 1] across cells (divided by the NORM_MAX_PCTILE percentile, no baseline
subtraction, so no expression stays 0) then averaged across the archetype's genes.
Normalization is computed within each panel's own cell subset, so panels are self-contained.
...
# Panels: (label, human Type, mouse marker TSV basename, archetype letters).
PANELS = [
...
def main():
    ...
    for col, (label, human_type, marker_file, letters) in enumerate(PANELS):
        ...
        # --- mouse markers -> human orthologs present in the data ---
        ...
        for i, letter in enumerate(letters):
            ...
            used = [mouse_to_human[g] for g in mouse_genes
                    if g in mouse_to_human and mouse_to_human[g] in human_genes]
            if not used:
                raise ValueError(f"No ortholog-mapped genes found in the data for "
                                 f"{label} {letter} ({arch_id}, {len(mouse_genes)} mouse markers)")
            print(f'  {letter} ({arch_id}): {len(used)}/{len(mouse_genes)} mouse markers mapped')
