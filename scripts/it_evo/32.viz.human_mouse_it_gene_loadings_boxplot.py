...
Mann-Whitney U stars vs the gray "other" bulk. See 24b for why boxes beat KDE marginals.
...
def mwu_vs_other(vals, S):
    ...
    for L in S['order']:
        ...
        out[L] = (p, 2 * u / (len(g[L]) * len(g['other'])) - 1)   # +1 = entirely above the bulk
    return out
...
def report_mwu(axis_label, ms, hs, S):
    for species, vals in (('mouse', ms), ('human', hs)):
        stats = mwu_vs_other(vals, S)
        parts = [f'{S["relabel"][L]}: med {np.median(group_values(vals, S)[L]):+.3f} '
                 f'rb {stats[L][1]:+.2f} p {stats[L][0]:.1e} {stars(stats[L][0])}'
                 for L in S['order'] if L in stats]
        print(f'    {S["label"]:5s} {axis_label} {species:5s} '
              f'(bulk med {np.median(group_values(vals, S)["other"]):+.3f})  ' + ' | '.join(parts))
...
def annotate_sig(ax, vals, orientation, S):
...
def draw_joint(subfig, S, axis_label, ms, hs, sig):
    ...
    annotate_sig(ax_boxx, ms, 'horizontal', S)
    annotate_sig(ax_boxy, hs, 'vertical', S)
    ...
    ax_boxy.set_xlabel('MWU vs "other"\nns * .05 ** .01 *** .001', fontsize=6)
