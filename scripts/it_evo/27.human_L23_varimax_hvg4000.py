"""Human Jorstad23 L2/3 IT — PCA + varimax + variance partitioning at N_HVG = 4000.

Standalone copy of `02.human_jorstad23_it_varimax.py` trimmed to the L2/3 subclass and
refit on the top-4000 HVGs instead of the top-2000 (plan/it_evo/10). Nothing in 02 is
edited: the N=2000 record stays valid by construction, and the two records coexist because
every output here carries an `_hvg4000` tag.

The recipe is otherwise byte-identical to 02 — raw-variance ranking of the log-normalized
matrix (`np.argsort(gene_var)[::-1][:N_HVG]`, NOT scanpy's dispersion-binned selection),
StandardScaler -> PCA(10, random_state=0) -> Kaiser varimax -> reorder by descending score
variance. `.X` is already log-normalized (ln(CPM+1)) and is used directly.

Note that VX NUMBERING IS NOT COMPARABLE TO 02's. Varimax components are ordered by score
variance, and that order changes with the gene set, so the Gate-A VX selection must be
re-derived from the new `27...vx_variance_partition_hvg4000.tsv` — the 02-era
`['VX2','VX6','VX7','VX8','VX9','VX10']` is meaningless in this basis.

Reads:
  links/it_evo/jorstad23_human_WithinArea_L23IT.h5ad
Outputs:
  local_data/res/it_evo/27.human_L23_varimax_coords_hvg4000.tsv
  local_data/res/it_evo/27.human_L23_varimax_loadings_hvg4000.tsv
  local_data/res/it_evo/27.human_L23_pc_variance_partition_hvg4000.tsv
  local_data/res/it_evo/27.human_L23_vx_variance_partition_hvg4000.tsv
  local_data/fig/it_evo/27.human_L23_pc_variance_partition_hvg4000.html
  local_data/fig/it_evo/27.human_L23_vx_variance_partition_hvg4000.html
"""
...
IN_HUMAN        = os.path.join(PROJECT_ROOT, 'links', 'it_evo',
                               'jorstad23_human_WithinArea_L23IT.h5ad')
OUT_VX_COORDS   = os.path.join(OUT_RES_DIR, '27.human_L23_varimax_coords_hvg4000.tsv')
OUT_VX_LOADINGS = os.path.join(OUT_RES_DIR, '27.human_L23_varimax_loadings_hvg4000.tsv')
OUT_PC_VAR_TSV  = os.path.join(OUT_RES_DIR, '27.human_L23_pc_variance_partition_hvg4000.tsv')
OUT_VX_VAR_TSV  = os.path.join(OUT_RES_DIR, '27.human_L23_vx_variance_partition_hvg4000.tsv')
OUT_PC_VAR_HTML = os.path.join(OUT_FIG_DIR, '27.human_L23_pc_variance_partition_hvg4000.html')
OUT_VX_VAR_HTML = os.path.join(OUT_FIG_DIR, '27.human_L23_vx_variance_partition_hvg4000.html')

# --- parameters ---
HUMAN_SUBCLASS   = 'L2/3 IT'
N_HVG            = 4000          # 02 uses 2000; this is the only analytical change
N_PCS            = 10
PCA_RANDOM_STATE = 0
...
print(f'{"=" * 70}\nL23 — human {HUMAN_SUBCLASS}  (N_HVG = {N_HVG})\n{"=" * 70}')

# --- 1. Load; .X is already log-normalized ---
print(f'Loading {IN_HUMAN}...')
adata = ad.read_h5ad(IN_HUMAN)
...
pc_var_df = variance_partition(scores, pc_cols, factors, 'L23 pre-varimax PCs')
pc_var_df.to_csv(OUT_PC_VAR_TSV, sep='\t')
print(f'Saved {OUT_PC_VAR_TSV}')
save_varpart_html(
    pc_var_df, pc_load_df, scores, pc_cols,
    f'Variance partitioning by PC — Jorstad23 human {HUMAN_SUBCLASS} '
    f'(pre-varimax, N_HVG={N_HVG})',
    OUT_PC_VAR_HTML,
)
...
vx_var_df = variance_partition(vx_scores, vx_cols, factors, 'L23 post-varimax VX')
vx_var_df.to_csv(OUT_VX_VAR_TSV, sep='\t')
print(f'Saved {OUT_VX_VAR_TSV}')
save_varpart_html(
    vx_var_df, vx_load_df, vx_scores, vx_cols,
    f'Variance partitioning by VX component — Jorstad23 human {HUMAN_SUBCLASS} '
    f'(post-varimax, N_HVG={N_HVG})',
    OUT_VX_VAR_HTML,
)

# --- 8. Save varimax coords (META_COLS carried for downstream scripts) ---
