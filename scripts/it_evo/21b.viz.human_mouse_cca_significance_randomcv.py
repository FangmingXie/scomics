"""Script 21 with the NAIVE (random-fold) CV drawn instead of the blocked one.

Identical to 21 in every other respect — same permutation nulls, same seed, same panels — but
the CV line and annotation read `r_cv_random` from 16's spectrum rather than `r_cv_blocked`.
Both numbers come from the same K-fold CV over genes; they differ only in how genes are
assigned to folds. Random folds deal genes out i.i.d., so a held-out gene usually has a
co-expressed partner in the training fold and the fitted axis "generalizes" to it for free;
blocked folds assign whole Ward co-expression modules to a fold, which removes that path.

This variant is for comparison, NOT for reporting: it is the optimistic number, and where the
two diverge the divergence is itself the finding (leakage through co-expression). Where they
agree, the drop from observed r to CV is plain estimation variance in the canonical weights,
not leakage. Read it side by side with 21 — never on its own.
...
Two figures from the same permutation nulls:
  A. a single detailed panel for L2/3 CCA1 — the headline conserved axis — with the full stats
     block (observed r, z, empirical p, null mean/sd/percentiles/max/skew, random-CV r, n);
  B. a grid of null distributions: rows are subclasses (L2/3, L4, L5IT, L6IT), columns are the
     top CCAs (CCA1..CCA4), each cell a compact null + observed + random-CV + z/p annotation.
...
exchangeability, which co-expression violates, so it is anti-conservative — and the CV drawn
here shares that weakness, since random folds are exchangeable in exactly the same way. 21's
blocked-CV r remains the honest generalization number.
...
Reads (per TOKEN):
  ...
  local_data/res/it_evo/16.<TOKEN>_axis_cca_spectrum.tsv   (random-CV r, for context)
  data/human_mouse_orthologs.tsv
Outputs:
  local_data/fig/it_evo/21b.human_mouse_L23_cca1_significance_randomcv.pdf  (detailed panel)
  local_data/fig/it_evo/21b.human_mouse_cca_significance_randomcv.pdf       (4×4 grid,
                                                                     per-panel x range)
  local_data/fig/it_evo/21b.human_mouse_cca_significance_randomcv_sharedx.pdf (4×4 grid, one
                                                                     x range + common bin edges
                                                                     for all panels — densities
                                                                     and positions comparable
...
OUT_DETAIL   = os.path.join(FIG_DIR, '21b.human_mouse_L23_cca1_significance_randomcv.pdf')
OUT_GRID     = os.path.join(FIG_DIR, '21b.human_mouse_cca_significance_randomcv.pdf')
OUT_GRID_SX  = os.path.join(FIG_DIR, '21b.human_mouse_cca_significance_randomcv_sharedx.pdf')
...
# --- config ---
# the one substantive difference from 21: which CV column of 16's spectrum is drawn.
CV_COLUMN = 'r_cv_random'    # 21 uses 'r_cv_blocked'
CV_LABEL  = 'random-CV r (naive, leaks)'
...
def analyze(cfg):
    """Observed spectrum, full permutation null, CV_COLUMN r, and n for one subclass."""
    ...
    k = obs.shape[0]
    cv = {i: float(spec.loc[f'CCA{i+1}', CV_COLUMN]) for i in range(k)}
...
def detail_panel(res, comp, out_path):
    ...
    ax.axvline(res['cv'][comp], color='#1f77b4', lw=1.8, ls='-.',
               label=f'{CV_LABEL} = {res["cv"][comp]:.3f}')
...
def grid_figure(results, out_path, shared_x=False):
    ...
    handles = [
        ...
        plt.Line2D([], [], color='#1f77b4', lw=1.4, ls='-.', label=CV_LABEL),
    ]
    ...
    fig.suptitle('Cross-species canonical correlations vs the gene-label permutation null '
                 ...
                 f'CV line is the NAIVE random-fold CV — leaks through co-expression; '
                 f'compare against 21 for the blocked-fold version   |   {scale_note}',
                 y=1.03, fontsize=12)
    fig.tight_layout()
    fig.savefig(out_path, bbox_inches='tight', dpi=300)
    plt.close(fig)
