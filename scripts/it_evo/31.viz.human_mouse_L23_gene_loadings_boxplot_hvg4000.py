"""Script 24b's boxplot figure rebuilt on the N_HVG = 4000 record.

Standalone copy of `24b.viz.human_mouse_L23_gene_loadings_boxplot.py` reading the N=4000
chain (27 human varimax, 28 mouse varimax, 29 extension, 30 CCA) instead of the N=2000 one
(02, it/19, 26, 16). The figure itself is unchanged: scatter of per-gene mouse vs human
canonical projections, coloured by mouse archetype, with one horizontal boxplot per group
below the x-axis and one vertical boxplot per group along the y-axis, plus Mann-Whitney U
stars against the gray "other" bulk. See 24b for why boxes beat the KDE marginals of 24.

WHAT IS AND IS NOT REFIT (plan/it_evo/10, decision 1). The AXES are re-estimated at N=4000.
The ARCHETYPE LABELS are not: `05.mouse_L23_archetype_markers.tsv` is read UNCHANGED, so the
colours are the N=2000 archetype definitions, fitted in the N=2000 mouse Gate-A subspace and
held fixed here. The figure therefore answers "where do the previously-established mouse
archetype programs sit on the newly-estimated conserved axes?" and says NOTHING about
whether the archetypes themselves are stable to gene selection — a separate question needing
its own design. Refitting them would in any case add zero coloured points: variance ranking
is nested (top-2000 ⊂ top-4000, verified 2000/2000 on both species), every mouse marker is a
mouse HVG at any N >= 2000, and `hvg_union` contains every mouse HVG, so marker coverage on
the union is saturated in N.

That saturation is enforced, not observed: EXPECT_ARCH_COUNTS is a fixed target on
`hvg_union`, and a mismatch means the gene set is built wrong rather than that the biology
moved.
...
and ordered A', B', C' (matching scripts/it/41,48,50 and 24b). Below, "A/B/C" names the
internal keys — including in EXPECT_ARCH_COUNTS.

Known limitation, pre-existing and equally true at N=2000: the colouring is mouse-only, so
the figure shows where the MOUSE archetype programs sit on the conserved axes and says
nothing about the human ones.
...
Reads:
  ...
  local_data/res/it_evo/29.{human,mouse}_L23_varimax_loadings_full_hvg4000.tsv
...
# Gate A at N_HVG=4000 — must match 30's HUMAN_VX/MOUSE_VX (plan/it_evo/10 §2). The N=2000
# sets are meaningless in this basis; varimax reorders components by score variance.
...
IN_H_HVG      = os.path.join(RES_DIR, f'27.human_L23_varimax_loadings{TAG}.tsv')
IN_M_HVG      = os.path.join(RES_DIR, f'28.mouse_L23_varimax_loadings{TAG}.tsv')
IN_H_FULL     = os.path.join(RES_DIR, f'29.human_L23_varimax_loadings_full{TAG}.tsv')
IN_M_FULL     = os.path.join(RES_DIR, f'29.mouse_L23_varimax_loadings_full{TAG}.tsv')
IN_W_HUMAN    = os.path.join(RES_DIR, f'30.L23_axis_cca_weights_human{TAG}{SUFFIX}.tsv')
...
# --- shared orthologs, centered gene loading blocks (mirrors 30.load_loadings) ---
...
# --- CCA1/CCA2 gene loadings from 30's canonical weights ---
...
# --- permutation significance of the cross-species canonical correlations (mirrors 24b) ---
...
def check_arch_counts(counts):
    """Fail-fast on the fixed marker-coverage target (plan/it_evo/10 Verification 4)."""
    expected = EXPECT_ARCH_COUNTS.get(UNIVERSE)
    if expected is None:
        return
    if counts != expected:
        raise ValueError(
            f'marker coverage on {UNIVERSE} is {counts}, expected {expected}. Decision 1 of '
            f'plan/it_evo/10 derives this analytically from HVG nesting, so a mismatch means '
            f'the gene set is built wrong — not that the biology moved.')
    print(f'  marker-coverage check OK: {counts} on {UNIVERSE}, identical to N=2000.')
...
def make_joint_figure(ms, hs, axis_label, out_pdf, sig):
    ...
    if not abs(abs(r) - sig['r_cca']) < R_MATCH_TOL:
        raise ValueError(
            ...
            f'correlation {sig["r_cca"]:.6f} that the permutation p-value tests — 30\'s saved '
    ...
    ax_main.set_title(f'L2/3 {axis_label}: mouse vs human  (N_HVG = 4000)\n'
                      f'r = {r:.3f},  {p_str}  (gene-label permutation, {N_PERM} reps)\n'
                      f'universe: {UNIVERSE},  {len(shared)} orthologous genes\n'
                      f'colours = N=2000 mouse archetype definitions, held fixed',
                      fontsize=10)
...
print('--- L2/3 CCA gene-loading joint plots, N_HVG=4000 (boxplot marginals) ---')
print(f'  universe: {UNIVERSE}  ({len(shared)} shared orthologs)')
print(f'  Gate A: human {HUMAN_VX}, mouse {MOUSE_VX}')
arch_counts = {L: int((letters == L).sum()) for L in ARCH_COLORS}   # internal keys
print(f'  archetype genes on scatter: '
      f'{ {ARCH_RELABEL[L]: arch_counts[L] for L in ARCH_ORDER} } '
      f'of {len(gene2arch)} mouse markers')
check_arch_counts(arch_counts)
