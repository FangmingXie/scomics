...
Replaces the top-k KNN assignment in script 12 with Sinkhorn-Knopp balanced
assignment to eliminate hub collapse. Human cells are first downsampled to 20%
(~9,425 cells) to keep the full similarity matrix tractable (~83 MB).
...
Output:
  local_data/res/l23_evo/16.mouse_sk_coords.tsv
  local_data/fig/l23_evo/16.mouse_sk_scatter.html
"""
...
OUT_MOUSE_COORDS = os.path.join(OUT_RES_DIR, '16.mouse_sk_coords.tsv')
OUT_HTML         = os.path.join(OUT_FIG_DIR, '16.mouse_sk_scatter.html')
...
SK_TAU             = 0.05   # temperature for exp(S / τ)
