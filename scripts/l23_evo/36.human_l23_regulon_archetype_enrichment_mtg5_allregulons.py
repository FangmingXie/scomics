...
Same method and inputs as scripts/l23_evo/35.human_l23_regulon_archetype_enrichment_mtg5.py
(mtg5 eRegulon list), but the heatmap shows EVERY regulon — no significance-based row
removal. All regulons with >= MIN_REGULON_GENES in-universe targets are drawn; cells are
still starred where FDR < STAR_FDR.
  ...
  - regulon targets  : links/l23_evo/regulon_gene_table_mtg5.tsv

The mtg5 table is already long-format (one row per regulon-gene pair), so no Excel
parsing (script 27) is needed. The 4 columns this analysis consumes
(regulon, TF, regulation_direction, Gene) are derived inline:
  regulation_direction = TF2G_sign/R2G_sign ; regulon = TF_direction.
NOTE: all mtg5 regulons are +/+ (activating); there are no -/+ (repressing) regulons,
so the repressing heatmap panel is empty.
...
Reads:
  ...
  links/l23_evo/regulon_gene_table_mtg5.tsv
Outputs:
  local_data/res/l23_evo/36.human_l23_regulon_archetype_enrichment_mtg5_allregulons.tsv  (long format)
  local_data/res/l23_evo/36.human_l23_enrichment_mtg5_allregulons_neglog10fdr.tsv          (matrix)
  local_data/res/l23_evo/36.human_l23_enrichment_mtg5_allregulons_log2or.tsv               (matrix)
  local_data/fig/l23_evo/36.human_l23_regulon_archetype_enrichment_mtg5_allregulons.html   (heatmap)
"""
...
REGULONS = os.path.join(PROJECT_ROOT, 'links', 'l23_evo', 'regulon_gene_table_mtg5.tsv')
...
STAR_FDR = 0.05          # heatmap cells marked '*' where FDR < this (no row removal)
...
def enrich():
    print('=== human L2/3 (full universe; mtg5 regulons; all regulons shown) ===')
    ...
    # mtg5 regulon targets (already long-format): derive the (regulon, TF,
    ...
    out_long = os.path.join(RES_DIR, '36.human_l23_regulon_archetype_enrichment_mtg5_allregulons.tsv')
    ...
    fdr_mat.to_csv(os.path.join(RES_DIR, '36.human_l23_enrichment_mtg5_allregulons_neglog10fdr.tsv'), sep='\t')
    or_mat.to_csv(os.path.join(RES_DIR, '36.human_l23_enrichment_mtg5_allregulons_log2or.tsv'), sep='\t')

    # ---- two heatmaps (+/+ and -/+), colored by log2 OR, '*' where FDR < STAR_FDR ----
    panels = [('+/+', 'activating'), ('-/+', 'repressing')]

    # collect each panel's matrices — EVERY regulon in that sign is shown (no removal)
    panel_show = []
    for sign, _lbl in panels:
        ...
        print(f'    {sign}: {len(order)} regulons shown (all)')

    if all(p is None for p in panel_show):
        print('    [skip heatmap] no regulons')
        return long
    ...
    fig.update_layout(
        title=f'Human L2/3 IT — archetype marker enrichment in regulons (mtg5, all regulons) '
              f'(full universe; log2 OR; * FDR<{STAR_FDR:g}; N={N} genes)',
        height=max(400, 18 * max_rows + 180), width=1000,
        **layout_caxes,
    )
    out_html = os.path.join(FIG_DIR, '36.human_l23_regulon_archetype_enrichment_mtg5_allregulons.html')
