"""Archetype scores for Cheng22 mouse L2/3 IT — seeded re-fit that also saves loadings.

Copy of script 21 with two changes: the PCHA fit is seeded (py_pcha draws its
...
script-18 varimax loadings. Script 21 and every 21.* output are kept as the original
record; because 21.* was fit unseeded, the vertices here will not match it exactly and
the downstream top-300 cell sets, markers, and scores differ numerically (qualitatively
the same). From this script on, the 58.* record is reproducible.
...
Outputs:
  ...
  local_data/res/l23_evo/58.mouse_pcha_xp.tsv
  local_data/res/l23_evo/58.mouse_pcha_aa.tsv
  local_data/res/l23_evo/58.mouse_pcha_inner_components.tsv
  local_data/res/l23_evo/58.mouse_pcha_inner_mean.tsv
  local_data/res/l23_evo/58.mouse_pcha_gene_loadings.tsv
  local_data/res/l23_evo/58.mouse_archetype_scores.tsv
  local_data/fig/l23_evo/58.mouse_archetype_scatter.html
  local_data/fig/l23_evo/58.mouse_archetype_scores.html
"""
...
OUT_MARKERS    = os.path.join(OUT_RES_DIR, '58.mouse_archetype_markers.tsv')
OUT_PCHA_XP    = os.path.join(OUT_RES_DIR, '58.mouse_pcha_xp.tsv')
OUT_PCHA_AA    = os.path.join(OUT_RES_DIR, '58.mouse_pcha_aa.tsv')
OUT_INNER_COMPS   = os.path.join(OUT_RES_DIR, '58.mouse_pcha_inner_components.tsv')
OUT_INNER_MEAN    = os.path.join(OUT_RES_DIR, '58.mouse_pcha_inner_mean.tsv')
OUT_GENE_LOADINGS = os.path.join(OUT_RES_DIR, '58.mouse_pcha_gene_loadings.tsv')
OUT_SCORES     = os.path.join(OUT_RES_DIR, '58.mouse_archetype_scores.tsv')
OUT_ARCH_HTML  = os.path.join(OUT_FIG_DIR, '58.mouse_archetype_scatter.html')
OUT_SCORE_HTML = os.path.join(OUT_FIG_DIR, '58.mouse_archetype_scores.html')
...
# Units are inherited from script 18 — weights on StandardScaler-z-scored log2(CP10k+1)
# HVG expression, which is not the scale of the X_norm matrix built below for
# markers/scores (that one is un-z-scored log2(CP10k+1)).
