"""Mouse B and C−A archetype scores on the seeded 58.* mouse PCHA embedding (plots only).

The script-55 figure rendered on the 58.* record — same panels, colormaps, percentiles and
display flip, only the input record differs. Scores, cell coords and archetype coords all
come from 58.*, so the figure is self-consistent with the seeded re-fit. Script 55 and
55.mouse_b_ca_scores.pdf are kept as the original record. No recomputation here.
...
Outputs:
  local_data/fig/l23_evo/59.mouse_b_ca_scores.pdf
"""
...
IN_SCORES    = os.path.join(OUT_RES_DIR, '58.mouse_archetype_scores.tsv')
IN_PCHA_XP   = os.path.join(OUT_RES_DIR, '58.mouse_pcha_xp.tsv')
IN_PCHA_AA   = os.path.join(OUT_RES_DIR, '58.mouse_pcha_aa.tsv')
OUT_PDF      = os.path.join(OUT_FIG_DIR, '59.mouse_b_ca_scores.pdf')
...
# PC2 = -PC2 ONLY (PC1 UNCHANGED), MATCHING 21.viz, 53.viz AND 55.viz, SO THE ARCHETYPE
# TRIANGLE IS ORIENTED IDENTICALLY IN ALL OF THEM. THE SAME SIGN FLIP IS APPLIED TO BOTH
