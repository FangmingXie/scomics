"""Barplot of each regulon's mean target-gene loading on the human CCA1 axis — wang25 L2/3 (V1+PFC).

Identical to script 61, but the human regulon target sets come from the SCENIC+ eRegulon table
computed on the wang25 L2/3 neurons combining the V1 and PFC samples
(links/l23_evo/regulon_gene_table_wang25_l23_v1pfc.tsv) instead of script 27's wang25-IT table.

For EVERY activating (+/+) human regulon in that table (unfiltered), this summarizes each regulon by
the MEAN, across its target genes, of each gene's loading on the human CCA1 axis. One bar per regulon,
sorted descending, colored by the regulon's mouse archetype enrichment (A/B/C vs other). Only regulons
with >= MIN_HVG_TARGETS target genes among the human HVGs (i.e. carrying a CCA1 loading) are plotted.

The eRegulon table is the raw SCENIC+ long form (columns TF, eRegulon_name, is_extended, TF2G_sign,
R2G_sign, Gene, ...). The (regulon, TF, Gene) columns this script consumes are derived inline as in
script 38: regulation_direction = TF2G_sign/R2G_sign, regulon = TF_direction, deduped on (regulon, Gene).
...
Reads:
  ...
  links/l23_evo/regulon_gene_table_wang25_l23_v1pfc.tsv         (human wang25 L2/3 V1+PFC eRegulons)
...
Outputs:
  ...
  local_data/fig/l23_evo/62.cca1_regulon_loadings_barplot_human_wang25l23v1pfc.pdf     (bar: all)
  local_data/fig/l23_evo/62.cca1_regulon_loadings_barplot_human_wang25l23v1pfc_AC.pdf  (bar: A/C)
  local_data/fig/l23_evo/62.cca1_regulon_loadings_boxplot_human_wang25l23v1pfc.pdf     (box: all)
  local_data/fig/l23_evo/62.cca1_regulon_loadings_boxplot_human_wang25l23v1pfc_AC.pdf  (box: A/C)
  local_data/fig/l23_evo/62.cca1_regulon_mean_loading_mouse_vs_human_wang25l23v1pfc_AC.pdf  (scatter:
      per-regulon mean target CCA1 loading, human vs mouse — mouse loadings from the mouse CCA1 weights)
"""
...
# Human regulon targets: raw SCENIC+ eRegulon table for wang25 L2/3 neurons (V1+PFC combined).
...
OUT_PDF_BAR    = os.path.join(OUT_FIG_DIR, '62.cca1_regulon_loadings_barplot_human_wang25l23v1pfc.pdf')
OUT_PDF_BOX    = os.path.join(OUT_FIG_DIR, '62.cca1_regulon_loadings_boxplot_human_wang25l23v1pfc.pdf')
OUT_PDF_BAR_AC = os.path.join(OUT_FIG_DIR, '62.cca1_regulon_loadings_barplot_human_wang25l23v1pfc_AC.pdf')
OUT_PDF_BOX_AC = os.path.join(OUT_FIG_DIR, '62.cca1_regulon_loadings_boxplot_human_wang25l23v1pfc_AC.pdf')
OUT_PDF_SCATTER = os.path.join(OUT_FIG_DIR, '62.cca1_regulon_mean_loading_mouse_vs_human_wang25l23v1pfc_AC.pdf')

# --- parameters ---
# The regulon set is UNFILTERED: every activating (+/+) regulon in the wang25 L2/3 V1+PFC eRegulon
# table is built programmatically below (name/mouse/human keys), not hardcoded.
REG_DIRECTION  = '_+/+'   # activating regulons only
# eRegulon table -> (regulon, TF, Gene): keep only these TF2G/R2G sign combos (as in script 38).
KEEP_DIRECTIONS = {'+/+'}
...
TITLE          = {
    'bar': 'wang25 L2/3 V1+PFC regulons — mean target-gene loading on human CCA1 (Jorstad23×Cheng22)',
    'box': 'wang25 L2/3 V1+PFC regulons — target-gene loading distribution on human CCA1 (Jorstad23×Cheng22)',
}
...
def criteria_lines(mode, show_log2or):
    ...
    lines = [
        'Regulons: all activating (+/+) wang25 L2/3 V1+PFC regulons',
...
# Human: derive (regulon, TF, Gene) from the raw wang25 L2/3 V1+PFC eRegulon table (script 38 style).
hreg = pd.read_csv(IN_HUMAN_REG, sep='\t')
hreg['regulation_direction'] = hreg['TF2G_sign'] + '/' + hreg['R2G_sign']
hreg = hreg[hreg['regulation_direction'].isin(KEEP_DIRECTIONS)].copy()
hreg['regulon'] = hreg['TF'] + '_' + hreg['regulation_direction']
hreg = hreg.drop_duplicates(subset=['regulon', 'Gene'])
...
def _draw_cca1_panel(ax, bar_df, mode, annot_fs, show_log2or, criteria_fs, label_fs=None):
    ...
    # criteria / annotation legend, placed OUTSIDE the axes to the right with a small font
    # (bbox_inches='tight' on save expands the canvas to include it)
...
def draw_ac_with_overlap(bar_df, out_pdf, mode, label_fs, annot_fs, width_factor=0.5, criteria_fs=5):
...
for mode, out_full, out_ac in [('bar', OUT_PDF_BAR, OUT_PDF_BAR_AC),
    ...
    draw_full(ordered, out_full, mode, label_fs=4, annot_fs=2.5, criteria_fs=5)
