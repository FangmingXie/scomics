"""Archetype number selection — Cheng22 mouse L2/3 IT (compute, refined w/ bootstrap error bars).

Refinement of 19.mouse_cheng22_num_archetype.py: instead of a single bootstrap
ARV per NOC, the bootstrap-ARV procedure is repeated N_OUTER times so the metrics
plot can show mean ± std error bars. EV and the per-group (rep) ARV stay single-valued.

Each outer repeat draws NREPEATS bootstrap resamples and reduces them to one ARV via
get_relative_variation; mean/std are taken across the N_OUTER repeats.

Reads:  local_data/res/l23_evo/18.mouse_varimax_coords.tsv
Outputs:
  local_data/res/l23_evo/19.refine.mouse_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/l23_evo/19.refine.mouse_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '19.refine.mouse_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '19.refine.mouse_num_archetype_plotdata.pkl')
