"""Archetype scores for Cheng22 mouse L2/3 IT — conservative one-vs-each Wilcoxon.

Replaces the one-vs-rest Wilcoxon in script 20 with a conservative one-vs-each test:
for every gene, run separate Wilcoxon tests against each other archetype, then keep
the max p-value and min log2FC (worst-case across pairwise comparisons). Uses those
stricter markers to compute a per-cell archetype score (normalized gene expression average).
...
Outputs:
  ...
  local_data/fig/l23_evo/21.mouse_archetype_scatter.html
  local_data/fig/l23_evo/21.mouse_archetype_scores.html
"""
...
OUT_MARKERS    = os.path.join(OUT_RES_DIR, '21.mouse_archetype_markers.tsv')
OUT_PCHA_XP    = os.path.join(OUT_RES_DIR, '21.mouse_pcha_xp.tsv')
OUT_PCHA_AA    = os.path.join(OUT_RES_DIR, '21.mouse_pcha_aa.tsv')
OUT_SCORES     = os.path.join(OUT_RES_DIR, '21.mouse_archetype_scores.tsv')
OUT_ARCH_HTML  = os.path.join(OUT_FIG_DIR, '21.mouse_archetype_scatter.html')
OUT_SCORE_HTML = os.path.join(OUT_FIG_DIR, '21.mouse_archetype_scores.html')
