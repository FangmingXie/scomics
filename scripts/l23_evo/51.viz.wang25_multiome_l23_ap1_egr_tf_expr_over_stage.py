"""
Companion to script 50 with the SAME two-panel layout, but showing each TF's OWN expression
over developmental stage (not its regulon-target activity), in L2/3 excitatory neurons
...
Each TF's per-cell expression is log2(CP10k+1) computed from raw counts (per-cell depth =
total counts across all genes). Cells are pooled by developmental stage (second trimester ->
third trimester -> infancy -> adolescence); each TF's per-stage means are min-max normalized
to [0, 1] across stages so lines are comparable in temporal shape. One figure, 2 panels
sharing a y-axis, in a single PDF.

Reads:
  links/l23_evo/wang25_human_multiome_gex_v1_l23.h5ad
Outputs:
  local_data/fig/l23_evo/51.wang25_multiome_l23_ap1_egr_tf_expr_over_stage.pdf
  local_data/res/l23_evo/51.wang25_multiome_l23_ap1_egr_tf_expr_over_stage.tsv
"""
...
# Panels: (label, list of TFs shown as separate lines) -- same set as script 50.
PANELS = [
...
OUT_PDF = os.path.join(OUT_FIG_DIR, '51.wang25_multiome_l23_ap1_egr_tf_expr_over_stage.pdf')
OUT_TSV = os.path.join(OUT_RES_DIR, '51.wang25_multiome_l23_ap1_egr_tf_expr_over_stage.tsv')


def tf_stage_stats(tf, expr, stage, stages):
    """Per-stage mean+/-SEM (and stage-level min-max) of one TF's log2(CP10k+1) expression."""
...
def main():
    ...
    # Fail-fast: all listed TF genes must be present.
    all_tfs = [tf for _, tfs in PANELS for tf in tfs]
    missing = [tf for tf in all_tfs if tf not in adata.var_names]
    if missing:
        raise ValueError(f"TF genes not found in dataset: {missing}")
    ...
    for col, (ax, (label, tfs)) in enumerate(zip(axes[0], PANELS)):
        for i, tf in enumerate(tfs):
            expr = np.log2(X[:, name2idx[tf]] / depth * CP_TARGET + 1.0)   # log2(CP10k+1)
            stats = tf_stage_stats(tf, expr, stage, stages)
            ...
            ax.errorbar(xpos, stats['mean_norm'].values, yerr=stats['sem_norm'].values,
                        ...
                        linewidth=1.5, elinewidth=1.0, label=tf)
        ...
        if col == 0:
            ax.set_ylabel('TF expression\nlog2(CP10k+1), min-max norm.', fontsize=8)
        ax.set_title(f'{label} TFs', fontsize=11)
        ax.legend(fontsize=8, frameon=False, title='TF', title_fontsize=8)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)

    fig.suptitle(f'AP-1 & EGR TF expression over developmental stage — {KEEP_TYPE} (V1)',
                 fontsize=12)
    fig.tight_layout()
