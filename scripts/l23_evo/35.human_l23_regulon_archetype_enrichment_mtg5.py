...
Identical method to scripts/l23_evo/30.human_l23_regulon_archetype_enrichment_allgenes.py,
but the regulon list is swapped to the mtg5 eRegulon table:
...
Outputs:
  local_data/res/l23_evo/35.human_l23_regulon_archetype_enrichment_mtg5.tsv  (long format)
  local_data/res/l23_evo/35.human_l23_enrichment_mtg5_neglog10fdr.tsv         (matrix)
  local_data/res/l23_evo/35.human_l23_enrichment_mtg5_log2or.tsv              (matrix)
  local_data/fig/l23_evo/35.human_l23_regulon_archetype_enrichment_mtg5.html  (heatmap)
"""
...
STAR_FDR = 0.05          # heatmap cells marked '*' where FDR < this; a regulon row is
                         # shown if FDR < this in >= 1 archetype (no log2 OR cutoff)
...
def enrich():
    print('=== human L2/3 (full universe; mtg5 regulons) ===')
    ...
    out_long = os.path.join(RES_DIR, '35.human_l23_regulon_archetype_enrichment_mtg5.tsv')
    ...
    fdr_mat.to_csv(os.path.join(RES_DIR, '35.human_l23_enrichment_mtg5_neglog10fdr.tsv'), sep='\t')
    or_mat.to_csv(os.path.join(RES_DIR, '35.human_l23_enrichment_mtg5_log2or.tsv'), sep='\t')

    # ---- two heatmaps (+/+ and -/+), colored by log2 OR, '*' where FDR < STAR_FDR ----
    panels = [('+/+', 'activating'), ('-/+', 'repressing')]

    # collect each panel's shown matrices (regulons with FDR < STAR_FDR in >= 1
    # archetype within that sign)
    panel_show = []
    for sign, _lbl in panels:
        ...
        print(f'    {sign}: {len(keep)} regulons '
              f'(FDR<{STAR_FDR:g} in >=1 archetype) shown')
    ...
    fig.update_layout(
        title=f'Human L2/3 IT — archetype marker enrichment in regulons (mtg5) '
              f'(full universe; log2 OR; * FDR<{STAR_FDR:g}; N={N} genes)',
        height=max(400, 18 * max_rows + 180), width=1000,
        **layout_caxes,
    )
    out_html = os.path.join(FIG_DIR, '35.human_l23_regulon_archetype_enrichment_mtg5.html')
