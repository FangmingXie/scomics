...
Same method and inputs as scripts/l23_evo/37.human_l23_regulon_archetype_enrichment_wang25_it_allregulons.py,
but the regulon targets come from the wang25 L2/3 (V1+PFC) eRegulon table instead of the wang25 IT table.
...
In both, cells are starred where FDR < STAR_FDR.
  ...
  - regulon targets  : links/l23_evo/regulon_gene_table_wang25_l23_v1pfc.tsv

The wang25 L2/3 (V1+PFC) table is already long-format (one row per regulon-gene pair; tab-separated),
so no Excel parsing (script 27) is needed. The 4 columns this analysis consumes (regulon, TF,
regulation_direction, Gene) are derived inline:
  regulation_direction = TF2G_sign/R2G_sign ; regulon = TF_direction.
NOTE: the L2/3 table contains both +/+ (activating) and -/+ (repressing) regulons, so both
...
Reads:
  ...
  links/l23_evo/regulon_gene_table_wang25_l23_v1pfc.tsv
Outputs:
  local_data/res/l23_evo/38.human_l23_regulon_archetype_enrichment_wang25_l23_v1pfc_allregulons.tsv  (long format)
  local_data/res/l23_evo/38.human_l23_enrichment_wang25_l23_v1pfc_allregulons_neglog10fdr.tsv          (matrix)
  local_data/res/l23_evo/38.human_l23_enrichment_wang25_l23_v1pfc_allregulons_log2or.tsv               (matrix)
  local_data/fig/l23_evo/38.human_l23_regulon_archetype_enrichment_wang25_l23_v1pfc_allregulons.html   (heatmap, all regulons)
  local_data/fig/l23_evo/38.human_l23_regulon_archetype_enrichment_wang25_l23_v1pfc_significant.html   (heatmap, FDR<STAR_FDR only)
"""
...
REGULONS = os.path.join(PROJECT_ROOT, 'links', 'l23_evo', 'regulon_gene_table_wang25_l23_v1pfc.tsv')
...
def draw_heatmap(long, N, significant_only, out_html, mode_label):
    ...
    fig.update_layout(
        title=f'Human L2/3 IT — archetype marker enrichment in regulons (wang25 L2/3 (V1+PFC), {mode_label}) '
...
def enrich():
    print('=== human L2/3 (full universe; wang25 L2/3 (V1+PFC) regulons; all regulons shown) ===')
    ...
    # wang25 L2/3 (V1+PFC) regulon targets (already long-format): derive the (regulon, TF,
    ...
    out_long = os.path.join(RES_DIR, '38.human_l23_regulon_archetype_enrichment_wang25_l23_v1pfc_allregulons.tsv')
    ...
    fdr_mat.to_csv(os.path.join(RES_DIR, '38.human_l23_enrichment_wang25_l23_v1pfc_allregulons_neglog10fdr.tsv'), sep='\t')
    or_mat.to_csv(os.path.join(RES_DIR, '38.human_l23_enrichment_wang25_l23_v1pfc_allregulons_log2or.tsv'), sep='\t')
    ...
    draw_heatmap(
        ...
        out_html=os.path.join(FIG_DIR, '38.human_l23_regulon_archetype_enrichment_wang25_l23_v1pfc_allregulons.html'),
    )
    draw_heatmap(
        long, N, significant_only=True, mode_label='significant regulons',
        out_html=os.path.join(FIG_DIR, '38.human_l23_regulon_archetype_enrichment_wang25_l23_v1pfc_significant.html'),
    )
    return long
