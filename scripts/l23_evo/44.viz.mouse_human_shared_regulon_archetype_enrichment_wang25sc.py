...
Identical to script 43, but the HUMAN panel uses a DIFFERENT wang25 regulon source: the
SCENIC+ eRegulon table parsed from Wang25 SuppTable13 (`27.human_wang25_regulon_targets.tsv`,
the source used by scripts 28 & 30 and by script 42), rather than the `wang25 IT`
`regulon_gene_table_wang25_it.csv` table used in script 37 / script 43.

Human enrichment is read from script 30's output (full expressed & non-uniform gene
universe, ~20.4k genes) so its background matches the mouse panel (script 41 reconstructs
the analogous expressed/nonzero-variance universe). Script 28 is the HVG-universe variant
of the same regulon source; swap IN_HUMAN to it if the 2000-HVG background is preferred.
...
Outputs:
  local_data/fig/l23_evo/44.mouse_human_shared_regulon_archetype_enrichment_wang25sc_allregulons.html
  local_data/fig/l23_evo/44.mouse_human_shared_regulon_archetype_enrichment_wang25sc_significant.html
"""
...
IN_HUMAN = os.path.join(RES_L23_DIR, '30.human_l23_regulon_archetype_enrichment_allgenes.tsv')
OUT_HTML_ALL = os.path.join(FIG_DIR, '44.mouse_human_shared_regulon_archetype_enrichment_wang25sc_allregulons.html')
OUT_HTML_SIG = os.path.join(FIG_DIR, '44.mouse_human_shared_regulon_archetype_enrichment_wang25sc_significant.html')
...
HUMAN_TITLE = 'Jorstad23 human L2/3 IT (wang25 SuppTable13 regulons)'
