...
CCA2 is shown unflipped (it_evo/18b reflects CCA1 only).
...
Two PDFs, both along CCA2 (same views as 40b):
  1. per-bin boxplots, mouse | human                    (one gene per page)
  2. gene x CCA2-bin heatmap per species, all genes in one figure — per-gene z-scored WITHIN
     each species, FIXED +/- ZLIM color limits (shared with 40b, so the two are directly
     comparable; |z| beyond ZLIM is clipped and the colorbar marks it with arrowheads), rows
     ordered once by hierarchical clustering (correlation distance, average linkage, optimal
     leaf ordering) of the concatenated mouse+human profile and applied to both panels.
...
# it_evo/18b reflects CCA1 only, so CCA2 is shown unflipped in both species.
MOUSE_CCA2_SIGN = 1.0
HUMAN_CCA2_SIGN = 1.0
AXIS_LABEL     = 'CCA2'
DPI            = 300
N_CCA2_BINS    = 10          # CCA2 bins for the boxplots and the heatmap
...
def select_bgenes():
    ...
    hs = Xc @ wdf_h.loc[CCA_AXIS, HUMAN_VX_COLS].to_numpy(dtype=float)
    ms = Yc @ wdf_m.loc[CCA_AXIS, MOUSE_VX_COLS].to_numpy(dtype=float)
...
def binmean_matrix(coord, expr_df, gene_list, species):
    ...
    rows = []
    for g in gene_list:
        _, means, _ = binned_stats_relative(coord, expr_df[g].values, N_CCA2_BINS)
...
def zscore_rows(mat, gene_list, species):
    ...
    if bad.size:
        raise ValueError(f'{species} genes with flat expression across all {AXIS_LABEL} bins '
                         f'(cannot z-score or cluster): {[gene_list[i] for i in bad]}')
...
def load_mouse_expr(genes_mouse, cell_index):
    ...
    if df.isna().any().any():
        raise ValueError('Mouse expression has NaN after reindexing to CCA2 barcodes (barcode mismatch).')
    return df
...
def load_human_expr(genes_human, cell_index):
    ...
    df = df.reindex(cell_index)
    if df.isna().any().any():
        raise ValueError('Human expression has NaN after reindexing to CCA2 barcodes (barcode mismatch).')
    return df
...
def draw_boxbins(ax, positions, groups, color, sym, sp_title):
    ...
    ax.set_xlabel(f'{AXIS_LABEL} bin (1..{N_CCA2_BINS}, low→high)')
...
print(f'Selecting the top {N_GENES} archetype-{ARCH_KEY} genes by {CCA_AXIS} contribution...')
ranked, genes, genes_human = select_bgenes()
ranked.to_csv(OUT_RANK_TSV, sep='\t', index=False)
print(f'Saved {OUT_RANK_TSV}')
print(f'  genes: {genes}')

# --- project cells onto the conserved CCA2 axis ---
print('Projecting cells onto the cross-species CCA2 axis...')
mouse_index, mouse_cca2, mouse_r = cca_axis(
    IN_MOUSE_VX, IN_MOUSE_W, MOUSE_VX_COLS, MOUSE_CCA2_SIGN, 'mouse')
human_index, human_cca2, human_r = cca_axis(
    IN_HUMAN_VX, IN_HUMAN_W, HUMAN_VX_COLS, HUMAN_CCA2_SIGN, 'human')
...
# --- expression matrices (aligned to CCA2 barcode order) ---
...
print(f'Writing {OUT_PDF_HEAT} ({len(genes)} genes x {N_CCA2_BINS} {AXIS_LABEL} bins)...')
...
mouse_scaled = zscore_rows(mouse_mat, genes, 'mouse')
human_scaled = zscore_rows(human_mat, genes_human, 'human')
...
fig.suptitle(f'Top {len(genes)} {ARCH_LABEL} genes by {AXIS_LABEL} contribution — mean expression '
             f'over {AXIS_LABEL} bins (r = {R_CCA2:.3f}; rows: hierarchical clustering)')
