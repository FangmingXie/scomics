...
Two heatmaps (mouse left, human right), same layout as script 37, showing the SAME set
of activating (+/+) regulons that are shared between the mouse and human L2/3 enrichment
analyses (matched by TF name, case-insensitively: mouse `Meis2_+/+` <-> human `MEIS2_+/+`).

Both panels are drawn from pre-computed enrichment long-tables (no recomputation here):
  - mouse : scripts/it/41 output for the L2/3 layer only (yoo25 regulons, 3 archetypes A'..C')
  - human : scripts/l23_evo/37 output (wang25 IT regulons, 4 archetypes A'..D')
...
its own BH-FDR from its source script). Design choices for this comparison figure:
  - Rows: the 45 shared TFs, ALIGNED (same row order in both panels) and ordered by the
    MOUSE dominant archetype (peak log2 OR), then descending mouse peak, so rows line up
    across species for reading.
...
NOTE: the human enrichment source (37, wang25 IT) differs from the wang25 regulon table used
in script 42 (script 27, `27.human_wang25_regulon_targets.tsv`); the shared set is defined by
what is present in these two enrichment tables after each script's >=5-in-universe-gene filter.
...
Reads:
  ...
  local_data/res/l23_evo/37.human_l23_regulon_archetype_enrichment_wang25_it_allregulons.tsv
Outputs:
  local_data/fig/l23_evo/43.mouse_human_shared_regulon_archetype_enrichment_allregulons.html
  local_data/fig/l23_evo/43.mouse_human_shared_regulon_archetype_enrichment_significant.html
"""
...
IN_HUMAN = os.path.join(RES_L23_DIR, '37.human_l23_regulon_archetype_enrichment_wang25_it_allregulons.tsv')
OUT_HTML_ALL = os.path.join(FIG_DIR, '43.mouse_human_shared_regulon_archetype_enrichment_allregulons.html')
OUT_HTML_SIG = os.path.join(FIG_DIR, '43.mouse_human_shared_regulon_archetype_enrichment_significant.html')
...
HUMAN_TITLE = 'Jorstad23 human L2/3 IT (wang25 IT regulons)'
