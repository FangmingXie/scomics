# First-pass analysis of Xenium mouse retina trial dataset.
...
OUT_RES_DIR          = os.path.join(PROJECT_ROOT, 'local_data', 'res', 'retina')
OUT_FIG_DIR          = os.path.join(PROJECT_ROOT, 'local_data', 'fig', 'retina')
OUT_H5AD             = os.path.join(OUT_RES_DIR, '00.first_pass.h5ad')
OUT_FIG_SPATIAL_ALL  = os.path.join(OUT_FIG_DIR, '00.spatial_all.html')
OUT_FIG_SPATIAL_QC   = os.path.join(OUT_FIG_DIR, '00.spatial_section_qc.html')
OUT_FIG_VIOLIN       = os.path.join(OUT_FIG_DIR, '00.qc_violin.html')
OUT_FIG_SCATTER      = os.path.join(OUT_FIG_DIR, '00.qc_scatter.html')
OUT_FIG_FDR          = os.path.join(OUT_FIG_DIR, '00.fdr_histogram.html')
OUT_FIG_UMAP_LEIDEN  = os.path.join(OUT_FIG_DIR, '00.umap_leiden.html')
OUT_FIG_UMAP_QC      = os.path.join(OUT_FIG_DIR, '00.umap_qc.html')

# --- Section filter (adjust based on OUT_FIG_SPATIAL_ALL) ---
SECTION_X_MIN = 5000
SECTION_X_MAX = 10000
SECTION_Y_MIN = 0
SECTION_Y_MAX = 4500
...
MIN_COUNTS = 10
MIN_GENES  = 5
MAX_FDR    = 0.30    # max fraction of non-gene-expression counts
...
# --- Spatial overview: all sections (50k subsample) ---
print('Plotting spatial overview (all sections)...')
rng     = np.random.default_rng(0)
sub_idx = rng.choice(adata.n_obs, size=min(50_000, adata.n_obs), replace=False)
sub_obs = adata.obs.iloc[sub_idx]

fig = go.Figure(go.Scattergl(
    x=sub_obs['x_centroid'], y=sub_obs['y_centroid'],
    mode='markers',
    marker=dict(size=2, color=sub_obs['total_counts'],
                colorscale='Viridis', colorbar=dict(title='total_counts'), showscale=True),
))
fig.add_vrect(x0=SECTION_X_MIN, x1=SECTION_X_MAX,
              fillcolor='red', opacity=0.08, line_width=1,
              annotation_text='selected x', annotation_position='top left')
fig.add_hrect(y0=SECTION_Y_MIN, y1=SECTION_Y_MAX,
              fillcolor='red', opacity=0.08, line_width=0,
              annotation_text='selected y', annotation_position='top right')
fig.update_yaxes(autorange='reversed')
fig.update_layout(title='All tissue sections — total_counts (50k subsample)',
                  xaxis_title='x (µm)', yaxis_title='y (µm)', height=700, width=850)
fig.write_html(OUT_FIG_SPATIAL_ALL)
print(f'  Saved → {OUT_FIG_SPATIAL_ALL}')
...
# --- Spatial QC: 3 side-by-side figures (each with its own colorbar) ---
...
# --- Scatter panels: cell_area vs n_genes | cell_area vs transcript_counts | cell_area vs nucleus_area ---
...
scatter_2d_categorical_html(
    ...
    title='UMAP — QC metrics (retina Xenium trial)',
