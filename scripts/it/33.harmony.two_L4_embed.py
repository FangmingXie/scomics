"""Harmony embedding — two-dataset L4 IT (load + merge + Harmony).
...
sweep lives in 33.harmony.two_L4_num_archetype.py (which reads this script's coords
output).

Procedure (mirrors template 28, on the merged object):
  1) per-dataset L4 extraction on the shared gene set (inner join)
...
Outputs:
  ...
  local_data/res/it/33.harmony.two_L4_pca_coords.tsv  (raw PCA, pre-Harmony)
  local_data/res/it/33.harmony.two_L4_loadings.tsv    (PCA gene loadings)
"""
...
OUT_HARMONY_COORDS = os.path.join(OUT_RES_DIR, '33.harmony.two_L4_coords.tsv')
OUT_PCA_COORDS     = os.path.join(OUT_RES_DIR, '33.harmony.two_L4_pca_coords.tsv')
OUT_PCA_LOADINGS   = os.path.join(OUT_RES_DIR, '33.harmony.two_L4_loadings.tsv')
