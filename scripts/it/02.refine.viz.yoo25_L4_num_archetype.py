"""Archetype number selection — Yoo25 mouse L4 IT (plots, refined w/ bootstrap error bars).

Reads the computed results from 02.refine.yoo25_L4_num_archetype.py and renders the
interactive HTML figures. The metrics plot shows bootstrap ARV and effective-EV
as mean ± std error bars. No computation here — rerun the compute script if inputs are missing.

Reads:
  local_data/res/it/02.refine.yoo25_L4_num_archetype_metrics.tsv
  local_data/res/it/02.refine.yoo25_L4_num_archetype_plotdata.pkl
Outputs:
  local_data/fig/it/02.refine.yoo25_L4_num_archetype_metrics.html
  local_data/fig/it/02.refine.yoo25_L4_num_archetype_interactive.html
"""
...
IN_METRICS_TSV   = os.path.join(OUT_RES_DIR, '02.refine.yoo25_L4_num_archetype_metrics.tsv')
IN_PLOTDATA      = os.path.join(OUT_RES_DIR, '02.refine.yoo25_L4_num_archetype_plotdata.pkl')
OUT_METRICS_HTML = os.path.join(OUT_FIG_DIR, '02.refine.yoo25_L4_num_archetype_metrics.html')
OUT_INTERACTIVE  = os.path.join(OUT_FIG_DIR, '02.refine.yoo25_L4_num_archetype_interactive.html')
...
save_metrics_err_plot_html(noc_grid, ev_grid, arv_mean, arv_std, av_rep_grid,
                           ...
                           f'Archetype selection — Yoo25 mouse L4 VX subspace (NDIM={NDIM})',
                           OUT_METRICS_HTML)
...
scatter_per_group_html(noc_grid, ev_grid, av_rep_grid, xp_grid, aa_reps_grid,
                       ...
                       f'Per-sample archetype overlay — Yoo25 mouse L4 VX subspace (NDIM={NDIM})',
                       OUT_INTERACTIVE)
