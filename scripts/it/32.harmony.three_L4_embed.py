"""Harmony embedding — tri-dataset L4 IT (load + merge + Harmony).
...
within dataset). This is the embedding half of the tri-dataset L4 expansion of
28.harmony.cheng22_L4_num_archetype.py; the archetype/NOC sweep lives in
32.harmony.three_L4_num_archetype.py (which reads this script's coords output).

Procedure (mirrors template 28, on the merged object):
  1) per-dataset L4 extraction on the shared gene set (inner join, 16,570 genes)
     - gao25 'L4/5 IT CTX Glut' -> L4; gao25 kept at a fixed-seed 25% subsample
...
Outputs:
  local_data/res/it/32.harmony.three_L4_coords.tsv      (H1..H10 + Type + Sample + Dataset)
  local_data/res/it/32.harmony.three_L4_pca_coords.tsv  (raw PCA, pre-Harmony)
  local_data/res/it/32.harmony.three_L4_loadings.tsv    (PCA gene loadings)
"""
...
OUT_HARMONY_COORDS = os.path.join(OUT_RES_DIR, '32.harmony.three_L4_coords.tsv')
OUT_PCA_COORDS     = os.path.join(OUT_RES_DIR, '32.harmony.three_L4_pca_coords.tsv')
OUT_PCA_LOADINGS   = os.path.join(OUT_RES_DIR, '32.harmony.three_L4_loadings.tsv')
...
DATASETS = [
    ...
    dict(tag='gao25', path=INPUT_GAO25, subclass_col='subclass_label',
         subclass_val='L4/5 IT CTX Glut', sample_col='donor_name', depth_col=None,
...
N_HVG          = 2000
N_PCS          = 10
GAO25_SUBSAMPLE_FRAC = 0.25   # keep this fraction of gao25 L4 cells (1.0 = keep all)
...
# --- 1. Load each dataset and filter to L4 ---
