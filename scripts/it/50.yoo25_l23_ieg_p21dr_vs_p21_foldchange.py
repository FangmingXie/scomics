"""Dark-rearing fold change of IEG expression per L2/3 archetype (Yoo25 AllAges mouse).

Adapts the l23_evo IEG-over-stage viz (48.viz.wang25_multiome_l23_ieg_expr_over_stage.py)
to the Yoo25 mouse L2/3 IT dark-rearing setting. Instead of tracing each immediate-early /
activity-regulated gene across developmental stages, this collapses to a single number per
(gene, archetype): the log2 fold change of expression between the two P21 conditions,
log2(P21DR / P21), i.e. how dark-rearing shifts each IEG within each L2/3 archetype.

Archetype membership (A/B/C, published A'/B'/C') is recomputed for the joint P21 + P21DR
L2/3 pool exactly as in scripts 48.v2 / 49 (script-34 marker sets, identical scoring recipe),
then each (age, archetype) is pseudobulked from the top N_TOP_CELLS purest cells and CPM
normalized. For each IEG gene g and archetype k:
...
Three panels are drawn (single PDF):
  - Heatmap: genes (rows) x archetypes A'/B'/C' (cols), diverging RdBu_r centered at 0.
  ...
  - Boxplot by regulon block: heatmap values pooled over genes x archetypes within each of the
    three contiguous row blocks (ROW_GROUPS: first 3 / middle / bottom 3 = A/B/C regulons).

Reads:
  links/it/superdupermegaRNA_yoo25_IT_AllAges.h5ad
  local_data/res/it/34.follow.two_L23_archetype_markers.tsv
Outputs:
  local_data/fig/it/50.yoo25_l23_ieg_p21dr_vs_p21_foldchange.pdf
  local_data/res/it/50.yoo25_l23_ieg_p21dr_vs_p21_foldchange.tsv
"""
...
# Gene panel matches SELECTED_REGULONS in script 48.v2 (developmental TFs, then AP-1/Egr IEGs,
# then Satb1/Tcf12/Jdp2), shown top-to-bottom in this fixed order.
GENES = ['Nfib', 'Rfx3', 'Meis2',
         'Fos', 'Fosb', 'Fosl2', 'Junb', 'Egr1', 'Egr2', 'Egr3', 'Egr4',
         'Satb1', 'Tcf12', 'Jdp2']

# Row-block grouping for the second boxplot: the fixed 48.v2 gene order splits into three
# contiguous blocks (first 3 / middle / bottom 3). Each block's box pools all its heatmap
# log2FC values across the three archetype columns.
ROW_GROUPS = [('A regulons', GENES[:3]),     # developmental TFs: Nfib, Rfx3, Meis2
              ('B regulons', GENES[3:-3]),   # AP-1/Egr IEG block: Fos..Egr4
              ('C regulons', GENES[-3:])]    # Satb1, Tcf12, Jdp2
...
OUT_PDF = os.path.join(OUT_FIG_DIR, '50.yoo25_l23_ieg_p21dr_vs_p21_foldchange.pdf')
OUT_TSV = os.path.join(OUT_RES_DIR, '50.yoo25_l23_ieg_p21dr_vs_p21_foldchange.tsv')
...
def main():
    ...
    # Fail-fast: all requested IEG genes must be present.
    missing = [g for g in GENES if g not in name2idx]
    if missing:
        raise ValueError(f"IEG genes not found in raw.var_names: {missing}")
    ...
    arch_labels = [ARCH_RELABEL[k] for k in ARCH_ORDER]  # ['A', 'B', 'C'] published as A',B',C'
    ...
    for gi, g in enumerate(GENES):
        for ai, k in enumerate(ARCH_ORDER):
            rows.append(dict(
                gene=g, archetype=ARCH_RELABEL[k], archetype_internal=k,
    ...
    fig, (ax_h, ax_b, ax_r) = plt.subplots(
        1, 3, figsize=(11.6, 0.42 * len(GENES) + 2.0),
    ...
    ax_h.set_yticklabels(GENES, fontsize=9)
    ...
    # black dividers between the three row blocks (A / B / C regulons)
    ...
    fig.suptitle(f'{KEEP_SUBCLASS} IT dark-rearing response by archetype (Yoo25 AllAges)',
                 fontsize=12)
