"""Mouse L5IT IT (cheng22, P28NR) PCA + varimax rotation + variance partitioning.

Applies the Cheng22 L2/3 varimax procedure (l23_evo/18) to cheng22, restricted to
the L5IT subclass. Produces varimax coords/loadings + pre/post-varimax variance
partition reports. The post-varimax VX variance partition (cell_type vs sample vs
library_size) drives the VX_COLS selection in 24.cheng22_L5IT_num_archetype.py.

Normalization: cheng22 .X is log1p(CP10k), so we re-normalize the integer raw counts
in .raw with log2(CP10k + 1) to stay faithful to the Cheng22 procedure.

Reads:
  links/it/superdupermegaRNA_cheng22_IT_P28NR.h5ad
Outputs:
  local_data/res/it/23.cheng22_L5IT_varimax_coords.tsv
  local_data/res/it/23.cheng22_L5IT_varimax_loadings.tsv
  local_data/res/it/23.cheng22_L5IT_pc_variance_partition.tsv
  local_data/fig/it/23.cheng22_L5IT_pc_variance_partition.html
  local_data/res/it/23.cheng22_L5IT_vx_variance_partition.tsv
  local_data/fig/it/23.cheng22_L5IT_vx_variance_partition.html
"""
...
OUT_VX_COORDS   = os.path.join(OUT_RES_DIR, '23.cheng22_L5IT_varimax_coords.tsv')
OUT_VX_LOADINGS = os.path.join(OUT_RES_DIR, '23.cheng22_L5IT_varimax_loadings.tsv')
OUT_PC_VAR_TSV  = os.path.join(OUT_RES_DIR, '23.cheng22_L5IT_pc_variance_partition.tsv')
OUT_PC_VAR_HTML = os.path.join(OUT_FIG_DIR, '23.cheng22_L5IT_pc_variance_partition.html')
OUT_VX_VAR_TSV  = os.path.join(OUT_RES_DIR, '23.cheng22_L5IT_vx_variance_partition.tsv')
OUT_VX_VAR_HTML = os.path.join(OUT_FIG_DIR, '23.cheng22_L5IT_vx_variance_partition.html')
...
save_varpart_html(
    ...
    'Variance partitioning by PC — cheng22 mouse L5IT IT (pre-varimax)',
    OUT_PC_VAR_HTML,
)
...
save_varpart_html(
    ...
    'Variance partitioning by VX component — cheng22 mouse L5IT IT (post-varimax)',
    OUT_VX_VAR_HTML,
)
