"""Archetype number selection on varimax subspace — cheng22 mouse L4 IT (compute).

Runs the PCHA NOC sweep on the varimax components selected from
21.cheng22_L4_vx_variance_partition.tsv (cell_type R² dominates over sample + library_size): ['VX1', 'VX4', 'VX5', 'VX6'].

This script only computes and persists results; plotting lives in
22.viz.cheng22_L4_num_archetype.py (reads the outputs below).

Reads:  local_data/res/it/21.cheng22_L4_varimax_coords.tsv
Outputs:
  local_data/res/it/22.cheng22_L4_num_archetype_metrics.tsv   (metric grids)
  local_data/res/it/22.cheng22_L4_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '22.cheng22_L4_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '22.cheng22_L4_num_archetype_plotdata.pkl')
