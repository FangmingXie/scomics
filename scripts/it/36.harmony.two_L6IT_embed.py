"""Harmony embedding — two-dataset L6IT (load + merge + Harmony).
...
sweep lives in 36.harmony.two_L6IT_num_archetype.py (which reads this script's coords
output).

Procedure (mirrors template 28, on the merged object):
  1) per-dataset L6IT extraction on the shared gene set (inner join)
...
Outputs:
  ...
  local_data/res/it/36.harmony.two_L6IT_pca_coords.tsv  (raw PCA, pre-Harmony)
  local_data/res/it/36.harmony.two_L6IT_loadings.tsv    (PCA gene loadings)
"""
...
OUT_HARMONY_COORDS = os.path.join(OUT_RES_DIR, '36.harmony.two_L6IT_coords.tsv')
OUT_PCA_COORDS     = os.path.join(OUT_RES_DIR, '36.harmony.two_L6IT_pca_coords.tsv')
OUT_PCA_LOADINGS   = os.path.join(OUT_RES_DIR, '36.harmony.two_L6IT_loadings.tsv')
