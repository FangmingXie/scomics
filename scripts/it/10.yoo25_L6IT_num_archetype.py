"""Archetype number selection on PCA subspace — Yoo25 mouse L6IT IT (compute).

Skips varimax / VX selection: runs the PCHA NOC sweep directly on the top 5 PCs
(PC1–PC5). Mirrors 04.yoo25_L4_num_archetype.py for the L6IT subclass.

This script only computes and persists results; plotting lives in
10.viz.yoo25_L6IT_num_archetype.py (reads the outputs below).

Reads:  local_data/res/it/09.yoo25_L6IT_pca_coords.tsv
Outputs:
  local_data/res/it/10.yoo25_L6IT_num_archetype_metrics.tsv   (metric grids)
  local_data/res/it/10.yoo25_L6IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '10.yoo25_L6IT_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '10.yoo25_L6IT_num_archetype_plotdata.pkl')
