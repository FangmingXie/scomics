"""Mouse L4 IT (Yoo25, P21) PCA + varimax rotation + variance partitioning.

Applies the Cheng22 L2/3 procedure (scripts l23_evo/18) to Yoo25 P21 data,
restricted to the L4 subclass. Produces varimax coords/loadings + pre/post-varimax
variance partition reports.

Normalization note: Yoo25 .X is already log1p(CP10k) (natural log), so to stay
faithful to the Cheng22 procedure we re-normalize the integer raw counts in .raw
with log2(CP10k + 1).

Reads:
  links/it/superdupermegaRNA_yoo25_IT_P21.h5ad
Outputs:
  local_data/res/it/01.yoo25_L4_varimax_coords.tsv
  local_data/res/it/01.yoo25_L4_varimax_loadings.tsv
  local_data/res/it/01.yoo25_L4_pc_variance_partition.tsv
  local_data/fig/it/01.yoo25_L4_pc_variance_partition.html
  local_data/res/it/01.yoo25_L4_vx_variance_partition.tsv
  local_data/fig/it/01.yoo25_L4_vx_variance_partition.html
"""
...
OUT_VX_COORDS   = os.path.join(OUT_RES_DIR, '01.yoo25_L4_varimax_coords.tsv')
OUT_VX_LOADINGS = os.path.join(OUT_RES_DIR, '01.yoo25_L4_varimax_loadings.tsv')
OUT_PC_VAR_TSV  = os.path.join(OUT_RES_DIR, '01.yoo25_L4_pc_variance_partition.tsv')
OUT_PC_VAR_HTML = os.path.join(OUT_FIG_DIR, '01.yoo25_L4_pc_variance_partition.html')
OUT_VX_VAR_TSV  = os.path.join(OUT_RES_DIR, '01.yoo25_L4_vx_variance_partition.tsv')
OUT_VX_VAR_HTML = os.path.join(OUT_FIG_DIR, '01.yoo25_L4_vx_variance_partition.html')
...
# Yoo25 .X is already log1p(CP10k); re-normalize the integer raw counts in .raw
# to match the Cheng22 procedure exactly.
print('Normalizing...')
...
save_varpart_html(
    ...
    'Variance partitioning by PC — Yoo25 mouse L4 IT (pre-varimax)',
    OUT_PC_VAR_HTML,
)
...
save_varpart_html(
    ...
    'Variance partitioning by VX component — Yoo25 mouse L4 IT (post-varimax)',
    OUT_VX_VAR_HTML,
)
