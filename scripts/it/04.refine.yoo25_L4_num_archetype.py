"""Archetype number selection on PCA subspace — Yoo25 mouse L4 IT (compute, refined).

Refinement of 04.yoo25_L4_num_archetype.py (top-5-PC baseline, no varimax): the
bootstrap-ARV procedure is repeated N_OUTER times so the metrics plot can show
mean ± std error bars. EV and the per-group (rep) ARV stay single-valued.

Reads:  local_data/res/it/03.yoo25_L4_pca_coords.tsv
Outputs:
  local_data/res/it/04.refine.yoo25_L4_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/04.refine.yoo25_L4_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_PC_COORDS    = os.path.join(OUT_RES_DIR, '03.yoo25_L4_pca_coords.tsv')
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '04.refine.yoo25_L4_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '04.refine.yoo25_L4_num_archetype_plotdata.pkl')
