"""Archetype-associated genes — two-dataset L2/3 IT, n=3 archetypes (follow-up to set 34).

Fixes NOC=3 (the L2/3 optimum from 34.harmony.two_L23_num_archetype.py) and identifies the
genes associated with each archetype, then computes per-cell archetype scores. Mirrors the
procedure in scripts/l23_evo/21.mouse_cheng22_archetype_scores.py, adapted to set 34's
...
Procedure:
  1) load Harmony coords; use H1..H5 as the PCHA feature space (matches set 34's N_ARCH_PCS=5)
...
Reads:
  local_data/res/it/34.harmony.two_L23_coords.tsv   (H1..H10 + Type + Sample + Dataset)
...
Outputs:
  ...
  local_data/res/it/34.follow.two_L23_pcha_xp.tsv
  local_data/res/it/34.follow.two_L23_pcha_aa.tsv
  local_data/res/it/34.follow.two_L23_archetype_scores.tsv
  local_data/fig/it/34.follow.two_L23_archetype_scatter.html
  local_data/fig/it/34.follow.two_L23_archetype_scores.html
"""
...
IN_COORDS      = os.path.join(OUT_RES_DIR, '34.harmony.two_L23_coords.tsv')
...
OUT_MARKERS    = os.path.join(OUT_RES_DIR, '34.follow.two_L23_archetype_markers.tsv')
OUT_PCHA_XP    = os.path.join(OUT_RES_DIR, '34.follow.two_L23_pcha_xp.tsv')
OUT_PCHA_AA    = os.path.join(OUT_RES_DIR, '34.follow.two_L23_pcha_aa.tsv')
OUT_SCORES     = os.path.join(OUT_RES_DIR, '34.follow.two_L23_archetype_scores.tsv')
OUT_ARCH_HTML  = os.path.join(OUT_FIG_DIR, '34.follow.two_L23_archetype_scatter.html')
OUT_SCORE_HTML = os.path.join(OUT_FIG_DIR, '34.follow.two_L23_archetype_scores.html')

# --- per-dataset config (mirrors 34.harmony.two_L23_embed.py) ---
DATASETS = [
    dict(tag='cheng22', path=INPUT_CHENG22, subclass_col='Subclass',
         subclass_val='L2/3', sample_col='Sample', depth_col='n_counts'),
    dict(tag='yoo25', path=INPUT_YOO25, subclass_col='Subclass',
         subclass_val='L2/3', sample_col='Sample', depth_col='total_counts'),
]
...
SAMPLE_COL      = 'Sample'
DATASET_COL     = 'Dataset'
H_COLS          = ['H1', 'H2', 'H3', 'H4', 'H5']   # top-5 Harmony PCs (set 34 N_ARCH_PCS=5)
NOC             = 3
NDIM            = len(H_COLS)   # 5
DROP_PCS        = []            # use all 5 Harmony PCs directly (no PC dropped), per set 34
N_TOP_CELLS     = 300
FRAC_IN_THRESH  = 0.25
FDR_THRESH      = 0.001
...
# --- 1. load each dataset, filter to L2/3 ---
print('Loading datasets and filtering to L2/3...')
adatas = {}
for d in DATASETS:
...
scatter_categorical_html(
    ...
    title=f'Two-dataset (cheng22+yoo25) L2/3 IT — Harmony PCHA space (NOC={NOC})',
...
gene_expr_scatter_html(
    ...
    title=f'Two-dataset (cheng22+yoo25) L2/3 IT — archetype scores (NOC={NOC})',
