"""L2/3 regulon-target activity across postnatal stages (Yoo25 AllAges mouse IT).

Visualize how a regulon's activity changes over postnatal development (P6 -> P21) in
L2/3 cortical excitatory neurons, with a separate overlaid line for the dark-reared (DR)
condition. Headline output: the Fosb (+/+) regulon activity vs stage.

Regulon activity per age (pseudobulk-by-Age, NOT per-cell — the Yoo25 AllAges axis is age):
...
Outputs:
  local_data/fig/it/46.yoo25_l23_regulon_target_activity_over_stage.pdf
  local_data/res/it/46.yoo25_l23_regulon_target_activity_over_stage.tsv
"""
...
SELECTED_REGULONS = ['Fosb']   # one panel per regulon; Fosb -> single panel
DIRECTION    = '+/+'           # activating regulons only (matches scripts 42/49)
SUBCLASS_COL = 'Subclass'
KEEP_SUBCLASS = 'L2/3'
AGE_COL      = 'Age'
CP_TARGET    = 1e6             # CPM
NCOLS        = 1               # panel grid columns (rows derived from n regulons)

OUT_PDF = os.path.join(OUT_FIG_DIR, '46.yoo25_l23_regulon_target_activity_over_stage.pdf')
OUT_TSV = os.path.join(OUT_RES_DIR, '46.yoo25_l23_regulon_target_activity_over_stage.tsv')
...
def main():
    ...
    # order ages by (day, condition) so normal precedes DR at each day
    ordered_ages = sorted(age_info, key=lambda a: (age_info[a][0], age_info[a][1]))
    ...
    rows, results = [], []   # results: (tf, activity_by_age_dict, n_used, n_total)
    for tf in SELECTED_REGULONS:
        ...
        for j, a in enumerate(ordered_ages):
            day, cond = age_info[a]
            act_by_age[a] = (day, cond, float(activity[j]))
            rows.append(dict(age=a, day=day, condition=cond,
                             n_cells=n_cells[a], regulon=tf, activity=float(activity[j])))
        results.append((tf, act_by_age, len(used), len(targets)))

    # --- figure: one panel per regulon; two overlaid lines (normal + DR) ---
    nrows = int(np.ceil(len(results) / NCOLS))
    fig, axes = plt.subplots(nrows, NCOLS, figsize=(5.2 * NCOLS, 3.6 * nrows), squeeze=False)
    axflat = axes.ravel()

    # x tick set = every distinct day across all ages; labels 'P<day>'
    all_days = sorted({age_info[a][0] for a in ordered_ages})
    cond_style = {'normal': dict(color='#4C72B0', marker='o', label='normal-reared'),
                  'DR':     dict(color='#C44E52', marker='s', label='dark-reared (DR)')}

    for ax, (tf, act_by_age, n_used, n_total) in zip(axflat, results):
        for cond in ['normal', 'DR']:
            pts = sorted((day, act) for _a, (day, c, act) in act_by_age.items() if c == cond)
            if not pts:
                continue
            xs = [p[0] for p in pts]
            ys = [p[1] for p in pts]
            ...
            ax.plot(xs, ys, '-', color=st['color'], marker=st['marker'],
                    markersize=6, linewidth=1.5, label=st['label'])
        ax.set_xticks(all_days)
        ax.set_xticklabels([f'P{d}' for d in all_days])
        ax.set_xlabel('postnatal day')
        ax.set_ylabel('regulon activity\n(mean [0,1] target expr.)', fontsize=9)
        ax.set_title(f'{tf} regulon ({n_used}/{n_total} targets)', fontsize=11)
    ...
    fig.suptitle(f'Regulon-target activity over postnatal stage — {KEEP_SUBCLASS} IT (Yoo25 AllAges)',
                 fontsize=12)
    fig.tight_layout()
