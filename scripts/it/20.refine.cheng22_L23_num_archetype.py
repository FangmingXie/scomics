"""Archetype number selection on varimax subspace — cheng22 mouse L2/3 IT (compute, refined).

Refinement of 20.cheng22_L23_num_archetype.py: the bootstrap-ARV procedure
is repeated N_OUTER times so the metrics plot can show mean ± std error bars. EV and the
per-group (rep) ARV stay single-valued. VX selection (cell_type R² dominates over sample + library_size): ['VX1', 'VX2', 'VX5', 'VX7', 'VX8', 'VX9'].

Reads:  local_data/res/it/19.cheng22_L23_varimax_coords.tsv
Outputs:
  local_data/res/it/20.refine.cheng22_L23_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/20.refine.cheng22_L23_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_VX_COORDS    = os.path.join(OUT_RES_DIR, '19.cheng22_L23_varimax_coords.tsv')
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '20.refine.cheng22_L23_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '20.refine.cheng22_L23_num_archetype_plotdata.pkl')

# --- parameters ---
CLUSTER_COL = 'Type'
SAMPLE_COL  = 'Sample'
# VX components selected from 19.cheng22_L23_vx_variance_partition.tsv
# (cell_type R² dominates over sample + library_size)
VX_COLS     = ['VX1', 'VX2', 'VX5', 'VX7', 'VX8', 'VX9']
NDIM        = len(VX_COLS) - 1
NOC_MIN     = 2
NOC_MAX     = 6
