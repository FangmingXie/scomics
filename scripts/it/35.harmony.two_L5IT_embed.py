"""Harmony embedding — two-dataset L5IT (load + merge + Harmony).

Integrates two mouse cortical IT datasets — cheng22 (P28NR) and yoo25 (P21) —
into a single Harmony-corrected embedding (batch = sample within dataset). This is
the gao25-free counterpart of 32.harmony.three_L4_embed.py; the archetype/NOC
sweep lives in 35.harmony.two_L5IT_num_archetype.py (which reads this script's coords
output).

Procedure (mirrors template 28, on the merged object):
  1) per-dataset L5IT extraction on the shared gene set (inner join)
     - sequencing depth comes from each dataset's validated total-counts column
       (cheng22 'n_counts', yoo25 'total_counts') BEFORE gene subsetting
...
Outputs:
  ...
  local_data/res/it/35.harmony.two_L5IT_pca_coords.tsv  (raw PCA, pre-Harmony)
  local_data/res/it/35.harmony.two_L5IT_loadings.tsv    (PCA gene loadings)
"""
...
OUT_HARMONY_COORDS = os.path.join(OUT_RES_DIR, '35.harmony.two_L5IT_coords.tsv')
OUT_PCA_COORDS     = os.path.join(OUT_RES_DIR, '35.harmony.two_L5IT_pca_coords.tsv')
OUT_PCA_LOADINGS   = os.path.join(OUT_RES_DIR, '35.harmony.two_L5IT_loadings.tsv')
