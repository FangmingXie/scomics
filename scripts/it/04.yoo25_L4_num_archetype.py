"""Archetype number selection on PCA subspace — Yoo25 mouse L4 IT (compute).

Same as 02.yoo25_L4_num_archetype.py but skips varimax / VX selection: runs the
PCHA NOC sweep directly on the top 5 PCs (PC1–PC5). Provides a baseline to compare
against the varimax-VX-subspace selection.

This script only computes and persists results; plotting lives in
04.viz.yoo25_L4_num_archetype.py (reads the outputs below).

Reads:  local_data/res/it/03.yoo25_L4_pca_coords.tsv
Outputs:
  local_data/res/it/04.yoo25_L4_num_archetype_metrics.tsv   (metric grids)
  local_data/res/it/04.yoo25_L4_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '04.yoo25_L4_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '04.yoo25_L4_num_archetype_plotdata.pkl')
