"""Fisher-exact enrichment of L5IT archetype markers in SCENIC+ regulons (mouse gao25 V1).

L5IT analog of scripts/it/42.gao25_l23_regulon_archetype_enrichment.py: same analysis,
with the regulon list swapped to the gao25 V1 L5IT eRegulon table and the archetype
markers/coords swapped to the L5IT set. For each archetype, test whether its marker gene
set is over-represented among each regulon's target genes (one-sided Fisher exact), then
render the archetype x regulon heatmap.

Two gene-set resources are combined:
  - archetype markers: local_data/res/it/35.follow.two_L5IT_archetype_markers.tsv
                       (mouse IT L5IT markers; cheng22 + yoo25, 2 archetypes)
  - regulon targets  : links/it/regulon_gene_table_gao25_v1l5.csv

The gao25 table is raw long-format (one row per regulon-gene pair). The 4 columns this
analysis consumes are derived inline (mirrors script 42):
  regulation_direction = TF2G_sign/R2G_sign ; regulon = TF_direction.
Only positive-R2G regulons ('+/+', '-/+') are kept.

Background universe: the L5IT *tested expression gene set* the marker Wilcoxon ran over
...
scripts/it/35.follow.two_L5IT_archetype_scores.py:127-176 (minus the PCHA/marker steps).
...
Reads:
  ...
  local_data/res/it/35.harmony.two_L5IT_coords.tsv
  links/it/regulon_gene_table_gao25_v1l5.csv
  links/it/superdupermegaRNA_cheng22_IT_P28NR.h5ad
  links/it/superdupermegaRNA_yoo25_IT_P21.h5ad
Outputs:
  local_data/res/it/44.gao25_L5IT_regulon_archetype_enrichment.tsv   (long format)
  local_data/res/it/44.gao25_L5IT_enrichment_neglog10fdr.tsv         (matrix)
  local_data/res/it/44.gao25_L5IT_enrichment_log2or.tsv              (matrix)
  local_data/fig/it/44.gao25_L5IT_regulon_archetype_enrichment.html             (significant-only heatmap)
  local_data/fig/it/44.gao25_L5IT_regulon_archetype_enrichment_allregulons.html (all-regulons heatmap)
"""
...
REGULONS = os.path.join(PROJECT_ROOT, 'links', 'it', 'regulon_gene_table_gao25_v1l5.csv')
...
# single L5IT config (markers / coords + subclass value + NOC); gao25 is V1 L5IT only
LAYER = dict(layer='L5IT', subclass_val='L5IT', noc=2,
...
def reconstruct_gene_universe(coords_index, subclass_val, adatas):
    ...
    embedding cells. Replicates scripts/it/35.follow.two_L5IT_archetype_scores.py:127-176.
...
def enrich_layer(cfg, adatas):
    ...
    # archetype display labels: default archetype_1->A, archetype_2->B, ...
    # (L5IT has no custom relabel; L2/3 is the only layer that overrides this)
    ...
    out_long = os.path.join(RES_DIR, f'44.gao25_{layer}_regulon_archetype_enrichment.tsv')
    ...
    fdr_mat.to_csv(os.path.join(RES_DIR, f'44.gao25_{layer}_enrichment_neglog10fdr.tsv'), sep='\t')
    or_mat.to_csv(os.path.join(RES_DIR, f'44.gao25_{layer}_enrichment_log2or.tsv'), sep='\t')
...
def render_heatmap(long, N, layer, show_all):
    ...
    fig.update_layout(
        title=f'gao25 V1 L5IT — archetype marker enrichment in regulons '
              f'(log2 OR; {scope}; N={N} genes)',
        height=max(400, 18 * max_rows + 180), width=1000,
        **layout_caxes,
    )
    suffix = '_allregulons' if show_all else ''
    out_html = os.path.join(FIG_DIR, f'44.gao25_{layer}_regulon_archetype_enrichment{suffix}.html')
