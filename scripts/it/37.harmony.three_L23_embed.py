"""Harmony embedding — tri-dataset L2/3 IT (load + merge + Harmony).
...
within dataset). This is the embedding half of the tri-dataset L2/3 expansion of
28.harmony.cheng22_L4_num_archetype.py; the archetype/NOC sweep lives in
37.harmony.three_L23_num_archetype.py (which reads this script's coords output).

Procedure (mirrors template 28, on the merged object):
  1) per-dataset L2/3 extraction on the shared gene set (inner join, 16,570 genes)
     - gao25 'L2/3 IT CTX Glut' -> L2/3; gao25 kept at a fixed-seed 25% subsample
...
Outputs:
  local_data/res/it/37.harmony.three_L23_coords.tsv      (H1..H10 + Type + Sample + Dataset)
  local_data/res/it/37.harmony.three_L23_pca_coords.tsv  (raw PCA, pre-Harmony)
  local_data/res/it/37.harmony.three_L23_loadings.tsv    (PCA gene loadings)
"""
...
OUT_HARMONY_COORDS = os.path.join(OUT_RES_DIR, '37.harmony.three_L23_coords.tsv')
OUT_PCA_COORDS     = os.path.join(OUT_RES_DIR, '37.harmony.three_L23_pca_coords.tsv')
OUT_PCA_LOADINGS   = os.path.join(OUT_RES_DIR, '37.harmony.three_L23_loadings.tsv')
...
DATASETS = [
    ...
    dict(tag='gao25', path=INPUT_GAO25, subclass_col='subclass_label',
         subclass_val='L2/3 IT CTX Glut', sample_col='donor_name', depth_col=None,
...
N_HVG          = 2000
N_PCS          = 10
GAO25_SUBSAMPLE_FRAC = 0.50   # keep this fraction of gao25 L2/3 cells (1.0 = keep all)
...
# --- 1. Load each dataset and filter to L2/3 ---
