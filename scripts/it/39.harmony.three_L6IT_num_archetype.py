"""Archetype number selection — tri-dataset L6IT (NOC sweep on Harmony PCs).

Reads the Harmony-corrected embedding produced by 39.harmony.three_L6IT_embed.py
...
No embedding here — rerun 39.harmony.three_L6IT_embed.py if the coords are missing.

Reads:
  local_data/res/it/39.harmony.three_L6IT_coords.tsv   (H1..H10 + Type + Sample + Dataset)
Outputs:
  local_data/res/it/39.harmony.three_L6IT_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/39.harmony.three_L6IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_HARMONY_COORDS  = os.path.join(OUT_RES_DIR, '39.harmony.three_L6IT_coords.tsv')
OUT_METRICS_TSV    = os.path.join(OUT_RES_DIR, '39.harmony.three_L6IT_num_archetype_metrics.tsv')
OUT_PLOTDATA       = os.path.join(OUT_RES_DIR, '39.harmony.three_L6IT_num_archetype_plotdata.pkl')
OUT_LOG            = os.path.join(OUT_LOG_DIR, '39.harmony.three_L6IT_num_archetype.log')
