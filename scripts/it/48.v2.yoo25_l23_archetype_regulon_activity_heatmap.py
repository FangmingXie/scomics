"""P21 vs P21DR archetype x regulon-activity heatmap — v2 (Yoo25 AllAges mouse L2/3 IT).
...
Archetype membership is recomputed here for the joint P21 + P21DR L2/3 pool from script 34's
marker gene sets using the identical scoring recipe (34.follow.two_L23_archetype_scores.py:227-244).

Pipeline:
  1. Regulon target sets (+/+ only): TF -> set of target symbols (fail-fast if any missing).
  2. Pool: subset AllAges to Subclass == 'L2/3' AND Age in {P21, P21DR}. Counts from adata.raw;
  ...
  5. Six pseudobulk columns (age-major, archetypes shown A',B',C'): per (age, k) take the top
  ...
  7. Per-row max-normalize (each regulon / its own max across the 6 cols). NO row clustering —
     rows stay in the fixed SELECTED_REGULONS order.
  8. RdBu_r heatmap (14 rows x 6 cols), P21|P21DR thin black divider, two-line column labels.
...
Outputs:
  local_data/fig/it/48.v2.yoo25_l23_archetype_regulon_activity_heatmap.pdf
  local_data/res/it/48.v2.yoo25_l23_archetype_regulon_activity_heatmap.tsv
  local_data/res/it/48.v2.yoo25_l23_p21_archetype_scores.tsv
"""
...
KEEP_SUBCLASS = 'L2/3'
AGE_COL      = 'Age'
AGES         = ['P21', 'P21DR']   # column-block order (normal first, then dark-reared)
...
OUT_PDF        = os.path.join(OUT_FIG_DIR, '48.v2.yoo25_l23_archetype_regulon_activity_heatmap.pdf')
OUT_TSV        = os.path.join(OUT_RES_DIR, '48.v2.yoo25_l23_archetype_regulon_activity_heatmap.tsv')
OUT_SCORES_TSV = os.path.join(OUT_RES_DIR, '48.v2.yoo25_l23_p21_archetype_scores.tsv')
...
def main():
    ...
    # --- six pseudobulk columns: (age, archetype), age-major, displayed A',B',C' order ---
    ...
    # --- min-max scale each gene to [0,1] jointly across the 6 columns ---
    ...
    # --- per-row max normalize (each regulon / its own max across 6 cols; peak->1) ---
    rmax = act_mat.max(axis=1, keepdims=True)
    act_norm = np.where(rmax > 0, act_mat / np.where(rmax > 0, rmax, 1.0), 0.0)

    # --- rows stay in the fixed SELECTED_REGULONS order (no clustering) ---
    row_labels = list(SELECTED_REGULONS)
    print(f'Row order (fixed): {row_labels}')
    ...
    # --- heatmap: regulons (fixed rows) x (age, archetype) columns ---
    ncols = len(columns)
    nrows = len(row_labels)
    fig, ax = plt.subplots(figsize=(0.7 * ncols + 2.4, 0.42 * nrows + 1.8))
    ...
    ax.set_title(f'Archetype x regulon activity — {KEEP_SUBCLASS} IT P21 vs P21DR '
