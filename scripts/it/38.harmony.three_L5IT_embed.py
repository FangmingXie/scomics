"""Harmony embedding — tri-dataset L5IT (load + merge + Harmony).
...
within dataset). This is the embedding half of the tri-dataset L5IT expansion of
28.harmony.cheng22_L4_num_archetype.py; the archetype/NOC sweep lives in
38.harmony.three_L5IT_num_archetype.py (which reads this script's coords output).

Procedure (mirrors template 28, on the merged object):
  1) per-dataset L5IT extraction on the shared gene set (inner join, 16,570 genes)
     - gao25 'L5 IT CTX Glut' -> L5IT; gao25 kept at a fixed-seed 25% subsample
...
Outputs:
  local_data/res/it/38.harmony.three_L5IT_coords.tsv      (H1..H10 + Type + Sample + Dataset)
  local_data/res/it/38.harmony.three_L5IT_pca_coords.tsv  (raw PCA, pre-Harmony)
  local_data/res/it/38.harmony.three_L5IT_loadings.tsv    (PCA gene loadings)
"""
...
OUT_HARMONY_COORDS = os.path.join(OUT_RES_DIR, '38.harmony.three_L5IT_coords.tsv')
OUT_PCA_COORDS     = os.path.join(OUT_RES_DIR, '38.harmony.three_L5IT_pca_coords.tsv')
OUT_PCA_LOADINGS   = os.path.join(OUT_RES_DIR, '38.harmony.three_L5IT_loadings.tsv')
...
DATASETS = [
    ...
    dict(tag='gao25', path=INPUT_GAO25, subclass_col='subclass_label',
         subclass_val='L5 IT CTX Glut', sample_col='donor_name', depth_col=None,
...
N_HVG          = 2000
N_PCS          = 10
GAO25_SUBSAMPLE_FRAC = 0.50   # keep this fraction of gao25 L5IT cells (1.0 = keep all)
...
# --- 1. Load each dataset and filter to L5IT ---
