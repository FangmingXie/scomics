"""Archetype number selection on varimax subspace — cheng22 mouse L4 IT (compute, refined).

Refinement of 22.cheng22_L4_num_archetype.py: the bootstrap-ARV procedure
is repeated N_OUTER times so the metrics plot can show mean ± std error bars. EV and the
per-group (rep) ARV stay single-valued. VX selection (cell_type R² dominates over sample + library_size): ['VX1', 'VX4', 'VX5', 'VX6'].

Reads:  local_data/res/it/21.cheng22_L4_varimax_coords.tsv
Outputs:
  local_data/res/it/22.refine.cheng22_L4_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/22.refine.cheng22_L4_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_VX_COORDS    = os.path.join(OUT_RES_DIR, '21.cheng22_L4_varimax_coords.tsv')
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '22.refine.cheng22_L4_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '22.refine.cheng22_L4_num_archetype_plotdata.pkl')

# --- parameters ---
CLUSTER_COL = 'Type'
SAMPLE_COL  = 'Sample'
# VX components selected from 21.cheng22_L4_vx_variance_partition.tsv
# (cell_type R² dominates over sample + library_size)
VX_COLS     = ['VX1', 'VX4', 'VX5', 'VX6']
NDIM        = len(VX_COLS) - 1
NOC_MIN     = 2
NOC_MAX     = 6
