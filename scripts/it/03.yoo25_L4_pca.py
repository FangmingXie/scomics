"""Mouse L4 IT (Yoo25, P21) PCA coords — no varimax (compute).
...
directly" archetype sweep in 04.yoo25_L4_num_archetype.py.

Normalization matches 01: Yoo25 .X is log1p(CP10k), so we re-normalize the integer
raw counts in .raw with log2(CP10k + 1) to stay faithful to the Cheng22 procedure.

The per-PC variance partition is identical to script 01's PCA front-end and already
lives in 01.yoo25_L4_pc_variance_partition.tsv/.html — not recomputed here.

Reads:
  links/it/superdupermegaRNA_yoo25_IT_P21.h5ad
Outputs:
  local_data/res/it/03.yoo25_L4_pca_coords.tsv
  local_data/res/it/03.yoo25_L4_pca_loadings.tsv
"""
...
OUT_PC_COORDS   = os.path.join(OUT_RES_DIR, '03.yoo25_L4_pca_coords.tsv')
OUT_PC_LOADINGS = os.path.join(OUT_RES_DIR, '03.yoo25_L4_pca_loadings.tsv')
