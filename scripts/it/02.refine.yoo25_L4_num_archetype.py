"""Archetype number selection — Yoo25 mouse L4 IT (compute, refined w/ bootstrap error bars).

Refinement of 02.yoo25_L4_num_archetype.py: instead of a single bootstrap ARV per
NOC, the bootstrap-ARV procedure is repeated N_OUTER times so the metrics plot can
show mean ± std error bars. EV and the per-group (rep) ARV stay single-valued.
...
Outputs:
  local_data/res/it/02.refine.yoo25_L4_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/02.refine.yoo25_L4_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '02.refine.yoo25_L4_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '02.refine.yoo25_L4_num_archetype_plotdata.pkl')
