"""Archetype number selection on PCA subspace — Yoo25 mouse L5IT IT (compute, refined).

Refinement of 08.yoo25_L5IT_num_archetype.py (top-5-PC baseline, no varimax):
the bootstrap-ARV procedure is repeated N_OUTER times so the metrics plot can show
mean ± std error bars. EV and the per-group (rep) ARV stay single-valued.

Reads:  local_data/res/it/07.yoo25_L5IT_pca_coords.tsv
Outputs:
  local_data/res/it/08.refine.yoo25_L5IT_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/08.refine.yoo25_L5IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_PC_COORDS    = os.path.join(OUT_RES_DIR, '07.yoo25_L5IT_pca_coords.tsv')
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '08.refine.yoo25_L5IT_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '08.refine.yoo25_L5IT_num_archetype_plotdata.pkl')
