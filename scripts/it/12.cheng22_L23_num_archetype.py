"""Archetype number selection on PCA subspace — cheng22 mouse L2/3 IT (compute).

Skips varimax / VX selection: runs the PCHA NOC sweep directly on the top 5 PCs
(PC1–PC5). Mirrors the yoo25 direct-PCA set for cheng22, L2/3 subclass.

This script only computes and persists results; plotting lives in
12.viz.cheng22_L23_num_archetype.py (reads the outputs below).

Reads:  local_data/res/it/11.cheng22_L23_pca_coords.tsv
Outputs:
  local_data/res/it/12.cheng22_L23_num_archetype_metrics.tsv   (metric grids)
  local_data/res/it/12.cheng22_L23_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '12.cheng22_L23_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '12.cheng22_L23_num_archetype_plotdata.pkl')
