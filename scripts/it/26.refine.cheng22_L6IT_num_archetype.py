"""Archetype number selection on varimax subspace — cheng22 mouse L6IT IT (compute, refined).

Refinement of 26.cheng22_L6IT_num_archetype.py: the bootstrap-ARV procedure
is repeated N_OUTER times so the metrics plot can show mean ± std error bars. EV and the
per-group (rep) ARV stay single-valued. VX selection (cell_type R² dominates over sample + library_size): ['VX1', 'VX2', 'VX9', 'VX10'].

Reads:  local_data/res/it/25.cheng22_L6IT_varimax_coords.tsv
Outputs:
  local_data/res/it/26.refine.cheng22_L6IT_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/26.refine.cheng22_L6IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_VX_COORDS    = os.path.join(OUT_RES_DIR, '25.cheng22_L6IT_varimax_coords.tsv')
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '26.refine.cheng22_L6IT_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '26.refine.cheng22_L6IT_num_archetype_plotdata.pkl')

# --- parameters ---
CLUSTER_COL = 'Type'
SAMPLE_COL  = 'Sample'
# VX components selected from 25.cheng22_L6IT_vx_variance_partition.tsv
# (cell_type R² dominates over sample + library_size)
VX_COLS     = ['VX1', 'VX2', 'VX9', 'VX10']
NDIM        = len(VX_COLS) - 1
NOC_MIN     = 2
NOC_MAX     = 6
