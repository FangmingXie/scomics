"""Archetype number selection — two-dataset L6IT (NOC sweep on Harmony PCs).

Reads the Harmony-corrected embedding produced by 36.harmony.two_L6IT_embed.py
...
scatter overlay. No embedding here — rerun 36.harmony.two_L6IT_embed.py if the coords
are missing.

Reads:
  local_data/res/it/36.harmony.two_L6IT_coords.tsv   (H1..H10 + Type + Sample + Dataset)
Outputs:
  local_data/res/it/36.harmony.two_L6IT_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/36.harmony.two_L6IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_HARMONY_COORDS  = os.path.join(OUT_RES_DIR, '36.harmony.two_L6IT_coords.tsv')
OUT_METRICS_TSV    = os.path.join(OUT_RES_DIR, '36.harmony.two_L6IT_num_archetype_metrics.tsv')
OUT_PLOTDATA       = os.path.join(OUT_RES_DIR, '36.harmony.two_L6IT_num_archetype_plotdata.pkl')
OUT_LOG            = os.path.join(OUT_LOG_DIR, '36.harmony.two_L6IT_num_archetype.log')
