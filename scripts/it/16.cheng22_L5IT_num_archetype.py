"""Archetype number selection on PCA subspace — cheng22 mouse L5IT IT (compute).

Skips varimax / VX selection: runs the PCHA NOC sweep directly on the top 5 PCs
(PC1–PC5). Mirrors the yoo25 direct-PCA set for cheng22, L5IT subclass.

This script only computes and persists results; plotting lives in
16.viz.cheng22_L5IT_num_archetype.py (reads the outputs below).

Reads:  local_data/res/it/15.cheng22_L5IT_pca_coords.tsv
Outputs:
  local_data/res/it/16.cheng22_L5IT_num_archetype_metrics.tsv   (metric grids)
  local_data/res/it/16.cheng22_L5IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '16.cheng22_L5IT_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '16.cheng22_L5IT_num_archetype_plotdata.pkl')
