"""Fisher-exact enrichment of L4 archetype markers in SCENIC+ regulons (mouse gao25 V1).

L4 analog of scripts/it/42.gao25_l23_regulon_archetype_enrichment.py: same analysis,
with the regulon list swapped to the gao25 V1 L4 eRegulon table and the archetype
markers/coords swapped to the L4 set. For each archetype, test whether its marker gene
...
Two gene-set resources are combined:
  - archetype markers: local_data/res/it/33.follow.two_L4_archetype_markers.tsv
                       (mouse IT L4 markers; cheng22 + yoo25, 3 archetypes)
  - regulon targets  : links/it/regulon_gene_table_gao25_v1l4.csv
...
Background universe: the L4 *tested expression gene set* the marker Wilcoxon ran over
...
Reads:
  ...
  local_data/res/it/33.harmony.two_L4_coords.tsv
  links/it/regulon_gene_table_gao25_v1l4.csv
  links/it/superdupermegaRNA_cheng22_IT_P28NR.h5ad
  links/it/superdupermegaRNA_yoo25_IT_P21.h5ad
Outputs:
  local_data/res/it/43.gao25_L4_regulon_archetype_enrichment.tsv   (long format)
  local_data/res/it/43.gao25_L4_enrichment_neglog10fdr.tsv         (matrix)
  local_data/res/it/43.gao25_L4_enrichment_log2or.tsv              (matrix)
  local_data/fig/it/43.gao25_L4_regulon_archetype_enrichment.html             (significant-only heatmap)
  local_data/fig/it/43.gao25_L4_regulon_archetype_enrichment_allregulons.html (all-regulons heatmap)
"""
...
REGULONS = os.path.join(PROJECT_ROOT, 'links', 'it', 'regulon_gene_table_gao25_v1l4.csv')
...
# single L4 config (markers / coords + subclass value + NOC); gao25 is V1 L4 only
LAYER = dict(layer='L4', subclass_val='L4', noc=3,
...
def enrich_layer(cfg, adatas):
    ...
    # (L4 has no custom relabel; L2/3 is the only layer that overrides this)
    ...
    out_long = os.path.join(RES_DIR, f'43.gao25_{layer}_regulon_archetype_enrichment.tsv')
    ...
    fdr_mat.to_csv(os.path.join(RES_DIR, f'43.gao25_{layer}_enrichment_neglog10fdr.tsv'), sep='\t')
    or_mat.to_csv(os.path.join(RES_DIR, f'43.gao25_{layer}_enrichment_log2or.tsv'), sep='\t')
...
def render_heatmap(long, N, layer, show_all):
    ...
    fig.update_layout(
        title=f'gao25 V1 L4 IT — archetype marker enrichment in regulons '
              f'(log2 OR; {scope}; N={N} genes)',
        height=max(400, 18 * max_rows + 180), width=1000,
        **layout_caxes,
    )
    suffix = '_allregulons' if show_all else ''
    out_html = os.path.join(FIG_DIR, f'43.gao25_{layer}_regulon_archetype_enrichment{suffix}.html')
