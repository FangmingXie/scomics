"""Archetype number selection — tri-dataset L4 IT (NOC sweep on Harmony PCs).

Reads the Harmony-corrected embedding produced by 32.harmony.three_L4_embed.py
...
No embedding here — rerun 32.harmony.three_L4_embed.py if the coords are missing.

Reads:
  local_data/res/it/32.harmony.three_L4_coords.tsv   (H1..H10 + Type + Sample + Dataset)
Outputs:
  local_data/res/it/32.harmony.three_L4_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/32.harmony.three_L4_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_HARMONY_COORDS  = os.path.join(OUT_RES_DIR, '32.harmony.three_L4_coords.tsv')
OUT_METRICS_TSV    = os.path.join(OUT_RES_DIR, '32.harmony.three_L4_num_archetype_metrics.tsv')
OUT_PLOTDATA       = os.path.join(OUT_RES_DIR, '32.harmony.three_L4_num_archetype_plotdata.pkl')
OUT_LOG            = os.path.join(OUT_LOG_DIR, '32.harmony.three_L4_num_archetype.log')
