"""Archetype number selection on varimax subspace — cheng22 mouse L6IT IT (compute).

Runs the PCHA NOC sweep on the varimax components selected from
25.cheng22_L6IT_vx_variance_partition.tsv (cell_type R² dominates over sample + library_size): ['VX1', 'VX2', 'VX9', 'VX10'].

This script only computes and persists results; plotting lives in
26.viz.cheng22_L6IT_num_archetype.py (reads the outputs below).

Reads:  local_data/res/it/25.cheng22_L6IT_varimax_coords.tsv
Outputs:
  local_data/res/it/26.cheng22_L6IT_num_archetype_metrics.tsv   (metric grids)
  local_data/res/it/26.cheng22_L6IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
OUT_METRICS_TSV = os.path.join(OUT_RES_DIR, '26.cheng22_L6IT_num_archetype_metrics.tsv')
OUT_PLOTDATA    = os.path.join(OUT_RES_DIR, '26.cheng22_L6IT_num_archetype_plotdata.pkl')
