"""Archetype number selection — tri-dataset L5IT (NOC sweep on Harmony PCs).

Reads the Harmony-corrected embedding produced by 38.harmony.three_L5IT_embed.py
and sweeps the number-of-archetypes (NOC) on the top 5 Harmony-corrected PCs
(H1-H5, NDIM=5), with N_OUTER bootstrap repeats for mean +/- std error bars.
Per-group (dataset:sample) archetypes are also fit for the scatter overlay.
No embedding here — rerun 38.harmony.three_L5IT_embed.py if the coords are missing.

Reads:
  local_data/res/it/38.harmony.three_L5IT_coords.tsv   (H1..H10 + Type + Sample + Dataset)
Outputs:
  local_data/res/it/38.harmony.three_L5IT_num_archetype_metrics.tsv   (metric grids w/ ARV mean/std)
  local_data/res/it/38.harmony.three_L5IT_num_archetype_plotdata.pkl  (proj + per-group archetypes)
"""
...
IN_HARMONY_COORDS  = os.path.join(OUT_RES_DIR, '38.harmony.three_L5IT_coords.tsv')
OUT_METRICS_TSV    = os.path.join(OUT_RES_DIR, '38.harmony.three_L5IT_num_archetype_metrics.tsv')
OUT_PLOTDATA       = os.path.join(OUT_RES_DIR, '38.harmony.three_L5IT_num_archetype_plotdata.pkl')
OUT_LOG            = os.path.join(OUT_LOG_DIR, '38.harmony.three_L5IT_num_archetype.log')
