"""Visualize NR-vs-DR DEGs across 4 equal-size VX3 bins (follow-up to script 45).
...
bin labels, condition, and log1p(CP10k) expression, then makes:
  1. Volcano plots (1x4 grid, one per bin) from the DEG tables.
...
Reads:
  ...
  local_data/res/astro/45.deg_vx3_bin{1..4}_NR_vs_DR_{all,sig}.tsv
Outputs:
  local_data/fig/astro/45.volcano_vx3_bins_NR_vs_DR.html
  local_data/fig/astro/45.boxplot_vx3_bins_NR_vs_DR.html
"""
...
OUT_VOLCANO        = os.path.join(FIG_DIR, '45.volcano_vx3_bins_NR_vs_DR.html')
OUT_BOXPLOT        = os.path.join(FIG_DIR, '45.boxplot_vx3_bins_NR_vs_DR.html')
...
# =============================================================================
# Step 2 — volcano plots (1x4 grid, one per bin)
# =============================================================================
...
for b in range(N_BINS):
    col = b + 1
    deg_df = pd.read_csv(os.path.join(DEG_DIR, f'45.deg_vx3_bin{b + 1}_NR_vs_DR_all.tsv'), sep='\t')
    deg_df = deg_df[deg_df['fdr'].notna()].copy()
...
for b in range(N_BINS):
    df_sig = pd.read_csv(os.path.join(DEG_DIR, f'45.deg_vx3_bin{b + 1}_NR_vs_DR_sig.tsv'), sep='\t')
