"""NR-vs-DR DEG analysis in 4 equal-size VX1 bins — PSEUDOBULK + DESeq2 (v6).

Same pipeline as v4 but (1) uses the all-archetype (Arch1-4) NR reference embedding
from script 41.v3, and (2) bins cells along VX1 (X_vx[:,0], the top-variance varimax
axis) instead of VX3. Aggregate raw counts per (sample x bin) -> one profile per mouse
per bin (7 NR + 8 DR), then run DESeq2 (via pydeseq2) per bin with negative-binomial
GLM, dispersion shrinkage, Wald test, Cook's/independent filtering, and apeglm-style
LFC shrinkage.
...
Outputs:
  ...
  local_data/res/astro/45.v6.deg_vx1_bin{1..4}_NR_vs_DR_sig.tsv
  local_data/res/astro/45.v6.deg_vx1_bins_nr_vs_dr_deseq2.log
"""
...
LOG_FILE           = os.path.join(OUT_DIR, '45.v6.deg_vx1_bins_nr_vs_dr_deseq2.log')
...
log = logging.getLogger('deg_vx1_v6')

# =============================================================================
# Step 1 — recompute bins (identical pipeline to script 45.v4, but VX1 axis)
# =============================================================================
...
log.info(f'Binning into {N_BINS} equal-size bins within each sample (VX1 axis)...')
...
for b in range(N_BINS):
    ...
    out_all = os.path.join(OUT_DIR, f'45.v6.deg_vx1_bin{b + 1}_NR_vs_DR_all.tsv')
    out_sig = os.path.join(OUT_DIR, f'45.v6.deg_vx1_bin{b + 1}_NR_vs_DR_sig.tsv')
