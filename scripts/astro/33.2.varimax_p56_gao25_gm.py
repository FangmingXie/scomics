"""Varimax rotation of PCA components + variance partitioning — P56 gao25 gray-matter astrocytes.
...
Fits PCA on P56 gray-matter astrocytes, applies Kaiser varimax rotation to gene loadings,
...
Outputs:
  local_data/res/astro/33.2.varimax_coords.tsv          (P56 GM × VX1..VX10 + metadata)
  local_data/res/astro/33.2.varimax_loadings.tsv        (HVGs × VX1..VX10)
  local_data/res/astro/33.2.pca_variance_partition.tsv  (PC1..PC10)
  local_data/res/astro/33.2.variance_partition.tsv      (VX1..VX10)
  local_data/res/astro/33.2.pca_components.tsv          (N_PCS × n_hvg)
  local_data/res/astro/33.2.pca_mean.tsv                (1 × n_hvg)
  local_data/res/astro/33.2.varimax_R.tsv               (N_PCS × N_PCS, reordered)
  local_data/fig/astro/33.2.variance_partition.html     (PCA + VX charts combined)
"""
...
OUT_VX_COORDS      = os.path.join(OUT_RES_DIR, '33.2.varimax_coords.tsv')
OUT_VX_LOADINGS    = os.path.join(OUT_RES_DIR, '33.2.varimax_loadings.tsv')
OUT_PCA_VAR_PARTITION = os.path.join(OUT_RES_DIR, '33.2.pca_variance_partition.tsv')
OUT_VAR_PARTITION  = os.path.join(OUT_RES_DIR, '33.2.variance_partition.tsv')
OUT_PCA_COMPONENTS = os.path.join(OUT_RES_DIR, '33.2.pca_components.tsv')
OUT_PCA_MEAN       = os.path.join(OUT_RES_DIR, '33.2.pca_mean.tsv')
OUT_VARIMAX_R      = os.path.join(OUT_RES_DIR, '33.2.varimax_R.tsv')
OUT_VAR_PART_HTML  = os.path.join(OUT_FIG_DIR, '33.2.variance_partition.html')
...
print(f'  P56 cells (before Arch4 removal): {p56_mask.sum()}')

# --- load arch labels and remove Arch4 from P56 cells ---
...
arch_labels_all = labels_p56['archetype'].values

non_arch4 = arch_labels_all != 'Arch4'
p56_indices = np.where(p56_mask)[0]
p56_gm_mask = np.zeros(len(ages), dtype=bool)
p56_gm_mask[p56_indices[non_arch4]] = True
arch_labels = arch_labels_all[non_arch4]
print(f'  Removed {(~non_arch4).sum()} Arch4 cells; P56 GM cells: {p56_gm_mask.sum()}')
print(f'  Arch1-3 distribution: {pd.Series(arch_labels).value_counts().to_dict()}')

# --- HVG selection (P56 GM cells only) ---
hvg_mask = select_hvg(x[p56_gm_mask], depths[p56_gm_mask], N_HVG)
...
xn_p56_gm = xn[p56_gm_mask]
print(f'  xn shape (all postnatal): {xn.shape}')

# --- fit PCA on P56 GM cells ---
print(f'Fitting PCA (N_PCS={N_PCS}) on P56 GM cells...')
pca = PCA(N_PCS, random_state=0)
pca.fit(xn_p56_gm)
pca_scores_p56 = pca.transform(xn_p56_gm)   # (n_p56_gm, N_PCS)
...
X_donor   = pd.get_dummies(donors[p56_gm_mask]).values.astype(float)
X_libsize = zscore(depths[p56_gm_mask]).reshape(-1, 1)
...
R = varimax(L)
vx_scores   = pca_scores_p56 @ R          # (n_p56_gm, N_PCS)
...
# --- save VX coords (P56 GM only) ---
vx_df = pd.DataFrame(vx_scores, index=adata.obs_names[p56_gm_mask], columns=vx_cols)
vx_df['archetype']  = arch_labels
vx_df['donor_name'] = donors[p56_gm_mask]
vx_df['Age']        = P56_AGE
vx_df['depth']      = depths[p56_gm_mask]
...
fig_pca.update_layout(
    barmode='stack',
    title='Variance partitioning by PCA component — P56 gao25 gray-matter astrocytes (no mt genes)',
...
fig_vx.update_layout(
    barmode='stack',
    title='Variance partitioning by varimax component — P56 gao25 gray-matter astrocytes (no mt genes)',
