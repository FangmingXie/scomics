# Archetype analysis — P56 astrocyte cells (all clusters), PC2 dropped, NOC=2 (Gao et al. 2025)
...
FIG_NOC_METRICS        = os.path.join(FIG_DIR, '11.archetype_p56_nopc2_noc2_metrics.png')
FIG_NOC_ARCHETYPES_REP = os.path.join(FIG_DIR, '11.archetype_p56_nopc2_noc2_interactive_rep.html')
FIG_NOC_PCA_CAT        = os.path.join(FIG_DIR, '11.archetype_p56_nopc2_noc2_pca_metadata.html')
FIG_GENE_SCATTER       = os.path.join(FIG_DIR, '11.archetype_p56_nopc2_noc2_gene_scatter.html')
...
save_metrics_plot(noc_grid, ev_grid, av_grid, av_rep_grid,
                  NDIM, f'Archetype selection P56 (no PC2) NOC={NOC_MIN}-{NOC_MAX} (NDIM={NDIM})', FIG_NOC_METRICS)

scatter_per_group_html(noc_grid, ev_grid, av_rep_grid, xp_grid, aa_reps_grid,
                       donors, donor_to_color,
                       f'Per-donor archetype overlay P56 (no PC2) NOC={NOC_MIN}-{NOC_MAX} (NDIM={NDIM})',
                       FIG_NOC_ARCHETYPES_REP)

scatter_categorical_html(
    xp_grid=xp_grid,
    cell_metadata={'cluster_label': types, 'donor_name': donors},
    title=f'P56 astrocytes (no PC2) NOC={NOC_MIN}-{NOC_MAX} — PCA scatter colored by metadata (NDIM={NDIM})',
...
gene_expr_scatter_html(
    ...
    gene_vals=gene_vals,
    title=f'P56 (no PC2) NOC={NOC_MIN}-{NOC_MAX} — gene expression',
