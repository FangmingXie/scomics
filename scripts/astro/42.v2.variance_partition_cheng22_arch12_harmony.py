"""Variance partitioning for cheng22 all P28+P38 Arch1+Arch2 PCA, Harmony, and VX components.

Same as script 42 but reads from the all-samples embedding (script 41.v2).

Reads:
  local_data/res/astro/41.v2.cheng22_arch12_harmony.h5ad
Outputs:
  local_data/res/astro/42.v2.pca_variance_partition.tsv
  local_data/res/astro/42.v2.harmony_variance_partition.tsv
  local_data/res/astro/42.v2.vx_variance_partition.tsv
  local_data/fig/astro/42.v2.variance_partition.html
"""
...
OUT_PCA_VAR_PART     = os.path.join(OUT_RES_DIR, '42.v2.pca_variance_partition.tsv')
OUT_HARMONY_VAR_PART = os.path.join(OUT_RES_DIR, '42.v2.harmony_variance_partition.tsv')
OUT_VX_VAR_PART      = os.path.join(OUT_RES_DIR, '42.v2.vx_variance_partition.tsv')
OUT_HTML             = os.path.join(OUT_FIG_DIR, '42.v2.variance_partition.html')
...
def stacked_bar_fig(vp_df, x_labels, title, xaxis_title):
    ...
    for factor, color in COLORS.items():
        if factor not in vp_df.columns:
            continue
...
pca_load       = np.array(adata.varm['PCA_loadings'], dtype=np.float64)
...
# --- covariate matrices (archetype, age, donor, library_size) ---
X_arch    = pd.get_dummies(arch_labels).values.astype(float)
X_age     = pd.get_dummies(ages).values.astype(float)
...
fig_pca = stacked_bar_fig(
    pca_vp, pc_x_labels,
    title='Variance partitioning — raw PCA (cheng22 all P28+P38, Arch1+Arch2)',
    xaxis_title='PCA component',
)
fig_hm = stacked_bar_fig(
    harmony_vp, hm_x_labels,
    title='Variance partitioning — Harmony-corrected PCA (cheng22 all P28+P38, Arch1+Arch2)',
    xaxis_title='Harmony component',
)
fig_vx = stacked_bar_fig(
    vx_vp, vx_x_labels,
    title='Variance partitioning — varimax of Harmony (cheng22 all P28+P38, Arch1+Arch2)',
