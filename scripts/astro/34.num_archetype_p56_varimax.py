"""Archetype number selection on varimax subspace — P56 gao25 astrocytes.

Runs PCHA NOC sweep on the analyst-selected VX column subspace.
VX_COLS and NDIM should be updated after inspecting script 33 variance partition HTML.

Reads:  local_data/res/astro/33.varimax_coords.tsv
Outputs:
  local_data/fig/astro/34.num_archetype_metrics.png
  local_data/fig/astro/34.num_archetype_interactive.html
"""
...
OUT_METRICS_PNG  = os.path.join(OUT_FIG_DIR, '34.num_archetype_metrics.png')
OUT_SCATTER_HTML = os.path.join(OUT_FIG_DIR, '34.num_archetype_interactive.html')

# analyst-set after inspecting 33.variance_partition.html
...
save_metrics_plot(noc_grid, ev_grid, av_grid, av_rep_grid,
                  NDIM, f'Archetype selection — P56 gao25 VX subspace (NDIM={NDIM})',
                  OUT_METRICS_PNG)

scatter_per_group_html(noc_grid, ev_grid, av_rep_grid, xp_grid, aa_reps_grid,
                       donors, donor_to_color,
                       f'Per-donor archetype overlay — P56 gao25 VX subspace (NDIM={NDIM})',
                       OUT_SCATTER_HTML)
