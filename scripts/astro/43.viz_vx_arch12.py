"""VX1-3 pairwise and 3D visualizations for cheng22 NR Arch1+Arch2 cells.
...
Outputs:
  local_data/fig/astro/43.vx_gene_expr.html   -- 2D pairs + 3D colored by top-VX gene expression
  local_data/fig/astro/43.vx_categorical.html  -- same layout colored by categorical labels
"""
...
OUT_GENE     = os.path.join(OUT_FIG_DIR, '43.vx_gene_expr.html')
OUT_CAT      = os.path.join(OUT_FIG_DIR, '43.vx_categorical.html')
...
# deduplicate while preserving per-component order (VX1 → VX2 → VX3), then append extras
seen = set()
ordered_genes = []
for comp in ['VX1', 'VX2', 'VX3']:
...
gene_expr_scatter_html(
    x=None, y=None,
    gene_vals=gene_vals,
    title='cheng22 NR Arch1+Arch2 — VX gene expression',
    out_path=OUT_GENE,
    xp=vx_scores,
    panels=panels,
    panel_3d=panel_3d,
...
scatter_categorical_html(
    ...
    title='cheng22 NR Arch1+Arch2 — VX categorical labels',
    out_path=OUT_CAT,
    panels=panels,
    panel_3d=panel_3d,
)
