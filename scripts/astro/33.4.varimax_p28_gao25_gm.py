"""Varimax rotation of PCA components + variance partitioning — P28 gao25 gray-matter astrocytes.

Same as script 33 with two additional filters applied from the outset:
  - Mitochondrial genes (mt-*) removed before HVG selection.
  - Arch4 (white-matter) cells excluded; analysis covers Arch1-3 gray-matter subtypes only.

Fits PCA on P28 gray-matter astrocytes, applies Kaiser varimax rotation to gene loadings,
partitions variance among Arch1-3, donor, and library_size factors for both the
original PCA components and the varimax-rotated components.
Saves the PCA+varimax parameters needed for algebraic projection in downstream scripts.

Reads:
  links/astro/gao25_scrna_astro.h5ad
  local_data/res/astro/17.labels_all_ages.parquet
Outputs:
  local_data/res/astro/33.4.varimax_coords.tsv          (P28 GM × VX1..VX10 + metadata)
  local_data/res/astro/33.4.varimax_loadings.tsv        (HVGs × VX1..VX10)
  local_data/res/astro/33.4.pca_variance_partition.tsv  (PC1..PC10)
  local_data/res/astro/33.4.variance_partition.tsv      (VX1..VX10)
  local_data/res/astro/33.4.pca_components.tsv          (N_PCS × n_hvg)
  local_data/res/astro/33.4.pca_mean.tsv                (1 × n_hvg)
  local_data/res/astro/33.4.varimax_R.tsv               (N_PCS × N_PCS, reordered)
  local_data/fig/astro/33.4.variance_partition.html     (PCA + VX charts combined)
"""
...
OUT_VX_COORDS      = os.path.join(OUT_RES_DIR, '33.4.varimax_coords.tsv')
OUT_VX_LOADINGS    = os.path.join(OUT_RES_DIR, '33.4.varimax_loadings.tsv')
OUT_PCA_VAR_PARTITION = os.path.join(OUT_RES_DIR, '33.4.pca_variance_partition.tsv')
OUT_VAR_PARTITION  = os.path.join(OUT_RES_DIR, '33.4.variance_partition.tsv')
OUT_PCA_COMPONENTS = os.path.join(OUT_RES_DIR, '33.4.pca_components.tsv')
OUT_PCA_MEAN       = os.path.join(OUT_RES_DIR, '33.4.pca_mean.tsv')
OUT_VARIMAX_R      = os.path.join(OUT_RES_DIR, '33.4.varimax_R.tsv')
OUT_VAR_PART_HTML  = os.path.join(OUT_FIG_DIR, '33.4.variance_partition.html')
...
p28_mask = ages == P28_AGE
print(f'  P28 cells (before Arch4 removal): {p28_mask.sum()}')

# --- load arch labels and remove Arch4 from P28 cells ---
labels_df = pd.read_parquet(IN_ARCH_LABELS)
labels_p28 = labels_df[labels_df['age'] == P28_AGE].reset_index(drop=True)
assert len(labels_p28) == p28_mask.sum(), (
    f'P28 label count mismatch: parquet {len(labels_p28)} vs adata {p28_mask.sum()}'
)
arch_labels_all = labels_p28['archetype'].values

non_arch4 = arch_labels_all != 'Arch4'
p28_indices = np.where(p28_mask)[0]
p28_gm_mask = np.zeros(len(ages), dtype=bool)
p28_gm_mask[p28_indices[non_arch4]] = True
arch_labels = arch_labels_all[non_arch4]
print(f'  Removed {(~non_arch4).sum()} Arch4 cells; P28 GM cells: {p28_gm_mask.sum()}')
print(f'  Arch1-3 distribution: {pd.Series(arch_labels).value_counts().to_dict()}')

# --- HVG selection (P28 GM cells only) ---
hvg_mask = select_hvg(x[p28_gm_mask], depths[p28_gm_mask], N_HVG)
...
xn_p28_gm = xn[p28_gm_mask]
print(f'  xn shape (all postnatal): {xn.shape}')

# --- fit PCA on P28 GM cells ---
print(f'Fitting PCA (N_PCS={N_PCS}) on P28 GM cells...')
pca = PCA(N_PCS, random_state=0)
pca.fit(xn_p28_gm)
pca_scores_p28 = pca.transform(xn_p28_gm)   # (n_p28_gm, N_PCS)
...
X_donor   = pd.get_dummies(donors[p28_gm_mask]).values.astype(float)
X_libsize = zscore(depths[p28_gm_mask]).reshape(-1, 1)
...
R = varimax(L)
vx_scores   = pca_scores_p28 @ R          # (n_p28_gm, N_PCS)
...
# save R with reorder baked in so downstream scripts can apply it directly
R_save = R[:, vx_var_order]               # (N_PCS, N_PCS)

# --- save VX coords (P28 GM only) ---
vx_df = pd.DataFrame(vx_scores, index=adata.obs_names[p28_gm_mask], columns=vx_cols)
vx_df['archetype']  = arch_labels
vx_df['donor_name'] = donors[p28_gm_mask]
vx_df['Age']        = P28_AGE
vx_df['depth']      = depths[p28_gm_mask]
...
# PCA bar chart — % of variance captured by the N_PCS components
...
fig_pca.update_layout(
    barmode='stack',
    title='Variance partitioning by PCA component — P28 gao25 gray-matter astrocytes (no mt genes)',
...
fig_vx.update_layout(
    barmode='stack',
    title='Variance partitioning by varimax component — P28 gao25 gray-matter astrocytes (no mt genes)',
