"""PCA + Harmony donor correction + varimax + UMAP + Leiden for cheng22 P28NR + P38NR astrocytes (all archetypes).

Same as script 41 but includes all archetypes (Arch1-4) instead of only Arch1+Arch2.
...
Outputs:
  ...
  local_data/fig/astro/41.v3.umap.html
  local_data/fig/astro/41.v3.pc12.html
  local_data/fig/astro/41.v3.vx12.html
"""
...
OUT_H5AD           = os.path.join(OUT_RES_DIR, '41.v3.cheng22_nr_harmony_arch1234.h5ad')
OUT_UMAP           = os.path.join(OUT_FIG_DIR, '41.v3.umap.html')
OUT_PC12           = os.path.join(OUT_FIG_DIR, '41.v3.pc12.html')
OUT_VX12           = os.path.join(OUT_FIG_DIR, '41.v3.vx12.html')

LABELED_AGES      = ['P28', 'P28_dl', 'P28_dr', 'P38', 'P38_dr']
NR_AGES           = ['P28', 'P38']
ARCH_KEEP         = ['Arch1', 'Arch2', 'Arch3', 'Arch4']
N_HVG             = 2000
N_PCS             = 10
MIN_CELLS         = 50
N_NEIGHBORS       = 30
...
# select NR cells (P28 + P38), then restrict to Arch1-4 (all archetypes)
arch_labels_all = labels_c22['archetype'].values
nr_mask_in_labeled = np.isin(ages[labeled_mask], NR_AGES)
nr_indices_all = np.where(labeled_mask)[0][nr_mask_in_labeled]
arch_labels_nr = arch_labels_all[nr_mask_in_labeled]

arch_keep_mask = np.isin(arch_labels_nr, ARCH_KEEP)
nr_indices = nr_indices_all[arch_keep_mask]
arch_labels = arch_labels_nr[arch_keep_mask]
print(f'  NR cells (P28 + P38): {len(nr_indices_all)}')
print(f'  Arch1-4 cells: {len(nr_indices)}')
print(f'  Archetype distribution: {pd.Series(arch_labels).value_counts().to_dict()}')

# --- HVG selection on Arch1-4 NR cells ---
...
# --- PCA on Arch1-4 NR cells ---
print(f'Fitting PCA (N_PCS={N_PCS}) on Arch1-4 NR cells...')
...
with open(OUT_UMAP, 'w') as f:
    f.write(_make_html(umap_coords, 'UMAP1', 'UMAP2',
                       'cheng22 NR astrocytes Arch1-4 — UMAP (Harmony corrected)',
                       'cheng22 NR astrocytes Arch1-4 — gene expression on UMAP'))
print(f'Saved {OUT_UMAP}')

print('Building PC1-2 HTML...')
with open(OUT_PC12, 'w') as f:
    f.write(_make_html(pca_scores[:, :2], 'PC1', 'PC2',
                       'cheng22 NR astrocytes Arch1-4 — PC1 vs PC2 (raw PCA)',
                       'cheng22 NR astrocytes Arch1-4 — gene expression on PC1-2'))
print(f'Saved {OUT_PC12}')

print('Building VX1-2 HTML...')
with open(OUT_VX12, 'w') as f:
    f.write(_make_html(vx_scores[:, :2], 'VX1', 'VX2',
                       'cheng22 NR astrocytes Arch1-4 — VX1 vs VX2 (varimax on Harmony)',
                       'cheng22 NR astrocytes Arch1-4 — gene expression on VX1-2'))
