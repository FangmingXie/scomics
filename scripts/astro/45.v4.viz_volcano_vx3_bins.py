"""Volcano plots of NR-vs-DR pseudobulk DESeq2 DEGs across 4 VX3 bins (v4).

Standalone: reads only the v4 DESeq2 tables (no per-cell pipeline needed for a
volcano). x-axis = shrunk log2FC (DESeq2 apeglm), y-axis = -log10(FDR / padj).
Genes with FDR = NaN (DESeq2 independent filtering / Cook's outliers) are dropped.
...
fig_v.update_layout(
    title=(f'NR vs DR pseudobulk DESeq2 DEGs across VX3 bins '
