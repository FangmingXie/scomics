"""PCA + varimax + UMAP + Leiden embedding for cheng22 P28NR + P38NR astrocytes.
...
Outputs:
  ...
  local_data/fig/astro/37.umap.html
"""
...
OUT_H5AD           = os.path.join(OUT_RES_DIR, '37.cheng22_nr.h5ad')
OUT_HTML           = os.path.join(OUT_FIG_DIR, '37.umap.html')
...
R = varimax(L)
vx_var_order = np.argsort((pca_scores @ R).var(axis=0))[::-1]
R = R[:, vx_var_order]
vx_scores   = pca_scores @ R        # (n_nr, N_PCS)
...
umap_coords = reducer.fit_transform(pca_scores)
...
nn.fit(pca_scores)
_, knn_indices = nn.kneighbors(pca_scores)
...
html1 = scatter_2d_categorical_html(
    ...
    title='cheng22 NR astrocytes (P28+P38) — UMAP',
    out_path=None,
...
html2 = gene_expr_scatter_html(
    ...
    gene_vals=gene_vals,
    title='cheng22 NR astrocytes — gene expression',
    out_path=None,
    xlabel='UMAP1', ylabel='UMAP2',
    return_html=True,
)
