...
Same as script 39 but restricted to Arch1 and Arch2 cells only.
...
OUT_H5AD           = os.path.join(OUT_RES_DIR, '41.cheng22_nr_harmony_arch12.h5ad')
...
# select NR cells (P28 + P38), then restrict to Arch1 and Arch2
...
arch12_mask = np.isin(arch_labels_nr, ARCH_KEEP)
nr_indices = nr_indices_all[arch12_mask]
arch_labels = arch_labels_nr[arch12_mask]
print(f'  NR cells (P28 + P38): {len(nr_indices_all)}')
print(f'  Arch1+Arch2 cells: {len(nr_indices)}')
print(f'  Archetype distribution: {pd.Series(arch_labels).value_counts().to_dict()}')

# --- HVG selection on Arch1+Arch2 NR cells ---
...
# --- PCA on Arch1+Arch2 NR cells ---
print(f'Fitting PCA (N_PCS={N_PCS}) on Arch1+Arch2 NR cells...')
...
with open(OUT_UMAP, 'w') as f:
    f.write(_make_html(umap_coords, 'UMAP1', 'UMAP2',
                       'cheng22 NR astrocytes Arch1+Arch2 — UMAP (Harmony corrected)',
                       'cheng22 NR astrocytes Arch1+Arch2 — gene expression on UMAP'))
print(f'Saved {OUT_UMAP}')

print('Building PC1-2 HTML...')
with open(OUT_PC12, 'w') as f:
    f.write(_make_html(pca_scores[:, :2], 'PC1', 'PC2',
                       'cheng22 NR astrocytes Arch1+Arch2 — PC1 vs PC2 (raw PCA)',
                       'cheng22 NR astrocytes Arch1+Arch2 — gene expression on PC1-2'))
print(f'Saved {OUT_PC12}')

print('Building VX1-2 HTML...')
with open(OUT_VX12, 'w') as f:
    f.write(_make_html(vx_scores[:, :2], 'VX1', 'VX2',
                       'cheng22 NR astrocytes Arch1+Arch2 — VX1 vs VX2 (varimax on Harmony)',
                       'cheng22 NR astrocytes Arch1+Arch2 — gene expression on VX1-2'))
