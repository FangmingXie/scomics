...
OUT_TSV_FILE        = os.path.join(OUT_RES_DIR, '19.scanpro_results.tsv')
OUT_FRAC_HTML       = os.path.join(OUT_FIG_DIR, '19.fraction_barplot.html')
...
# --- filter to EN-L2-3-CTX cells and L2/3 subtypes only ---
df = df[df['celltype'] == KEEP_CELLTYPE].copy()
df = df[df[PRED_TYPE_COL].isin(PRED_TYPES)].copy()
print(f'  {len(df)} cells after filtering to celltype={KEEP_CELLTYPE!r} and pred_types={PRED_TYPES}')
...
stacked_bar_html(
    ...
    title=f'Predicted L2/3 subtype fractions — {KEEP_CELLTYPE} only (cheng22_yoo25 → morcom26)',
